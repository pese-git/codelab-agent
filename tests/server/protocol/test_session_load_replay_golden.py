...
Предусловие шага 3 ADR-008 (журнал доменных событий, ACP — проекция). Сегодня реплей —
**pass-through**: `SessionReplayer` отдаёт элементы `events_history` в нотификацию дословно,
без конверсии. Шаг 3 заменяет его проекцией из доменного журнала, и проекция обязана дать
байт-идентичный поток: иначе клиент после `session/load` увидит другую историю, чем видел в
turn-е, а у части клиентов это разъезжается с их собственным состоянием.
...
* `replay_history` — события журнала; здесь и живёт pass-through;
...
**Зафиксирована и известная асимметрия:** `session_info_update` писатель умеет, а
`_REPLAYABLE_UPDATE_TYPES` его не перечисляет — событие персистируется и не реплеится никогда.
Гейт закрепляет это как **текущее** поведение, а не как правильное: судьба поля решается
шагом 3 (см. ADR-008). Если решение будет «реплеить», этот тест обязан поменяться осознанно.
"""
...
class TestReplayHistoryGolden:
    ...
    def test_session_info_update_is_not_replayed(self, replayer: SessionReplayer) -> None:
        ...
        Закрепляется **текущее** поведение: писатель умеет шесть видов,
        `_REPLAYABLE_UPDATE_TYPES` перечисляет пять. Судьбу решает шаг 3 ADR-008 —
        либо событие начинает реплеиться, либо перестаёт писаться, и тогда этот тест
        меняется осознанно, а не «сам сломался».
        """
        session = _session_with_all_event_kinds()
