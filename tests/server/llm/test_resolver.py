...
class TestModelResolverSessionCache:
    ...
    async def test_resolve_for_session_caches_provider(
        ...
        provider1, model1 = await resolver.resolve_for_session("sess_1", "openai/gpt-4o")
        provider2, model2 = await resolver.resolve_for_session("sess_1", "openai/gpt-4o")
    ...
    async def test_resolve_for_session_different_sessions(
        ...
        provider1, _ = await resolver.resolve_for_session("sess_1", "openai/gpt-4o")
        provider2, _ = await resolver.resolve_for_session("sess_2", "openai/gpt-4o")

        # Разные сессии — разные экземпляры (первый вызов для каждой)
        # Но после кэширования должны совпадать
        provider1_again, _ = await resolver.resolve_for_session("sess_1", "openai/gpt-4o")
    ...
    async def test_invalidate_session_clears_cache(
        ...
        provider2, _ = await resolver.resolve_for_session("sess_1", "openai/gpt-4o")
        assert "sess_1" in resolver._session_cache
    ...
    async def test_resolve_for_session_with_model_ref_object(
        ...
        ref = ModelRef(provider_id="anthropic", model_id="claude-sonnet-4")
        provider, model_id = await resolver.resolve_for_session("sess_1", ref)
        assert model_id == "claude-sonnet-4"
        assert provider.name == "anthropic"
    ...
    async def test_resolve_for_session_cache_miss_on_different_model(
        ...
        provider2, model2 = await resolver.resolve_for_session("sess_1", "openai/gpt-4o-mini")
