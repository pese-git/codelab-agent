...
# ===========================================================================
# Тесты singleton lock механизма
# ===========================================================================

def test_lock_file_functions() -> None:
    """Тестирует функции работы с lock файлом."""
    from codelab.server.transport.stdio_runner import (
        _LOCK_FILE,
    ...
    # Убеждаемся что lock файл не существует
    _release_singleton_lock()
    ...
    # Проверяем что PID записан
    pid = _read_pid_from_lock()
    assert pid == os.getpid()
    assert _is_process_running(pid)

    # Проверяем что несуществующий процесс не запущен
    assert not _is_process_running(999999)
...
def test_acquire_lock_no_file() -> None:
    """Тестирует что _acquire_singleton_lock работает если файла нет."""
    from codelab.server.transport.stdio_runner import (
        _acquire_singleton_lock,
        _release_singleton_lock,
    )
    ...
    _release_singleton_lock()
    # Должно успешно захватить блокировку
    assert _acquire_singleton_lock() is True
    _release_singleton_lock()


def test_acquire_lock_stale_pid() -> None:
    """Тестирует что stale lock файл обрабатывается корректно."""
    from codelab.server.transport.stdio_runner import (
        _LOCK_FILE,
        _acquire_singleton_lock,
        _release_singleton_lock,
    ...
    # Создаём lock файл с несуществующим процессом
    _LOCK_FILE.parent.mkdir(parents=True, exist_ok=True)
    _LOCK_FILE.write_text("999999")

    # Должно успешно захватить блокировку (stale PID не блокирует)
    assert _acquire_singleton_lock() is True
    _release_singleton_lock()
