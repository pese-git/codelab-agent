...
**Почему три отдельных компонента, а не один "smart reader"?**

Единый "smart reader" имел бы проблемы:
- **Нарушение SRP** — один класс отвечал бы за чтение диапазонов, обработку больших файлов и обрезку контекста
- **Сложность тестирования** — сложно mock'ать отдельные части
- **Сложность расширения** — добавление новой функциональности требует изменения большого класса

Три компонента:
- **ReadRangeStrategy** — чтение диапазонов (что читать)
- **LargeFileHandler** — обработка больших файлов (как читать)
- **ContextPruner** — обрезка нерелевантного (что удалить)

Каждый компонент решает свою задачу, тестируется независимо, расширяется без изменения других.

**Почему LargeFileHandler использует extract signatures + read relevant + summarize irrelevant, а не просто truncate?**

Простой truncate (обрезание конца файла) имеет проблемы:
- **Теряет важную информацию** — конец файла часто содержит реализацию
- **Не понимает структуру** — обрезает посередине функции или класса
- **Нет контекста** — LLM не понимает, что было обрезано

LargeFileHandler:
- **Extract signatures** — извлекает определения классов, функций, интерфейсов (всегда важны)
- **Read relevant** — читает релевантные части (поиск по ключевым словам)
- **Summarize irrelevant** — суммаризирует нерелевантные части через LLM (сохраняет контекст)

Результат: LLM получает полную картину (signatures + relevant + summary), а не обрезанный файл.

**Почему ContextPruner удаляет comments/empty lines/unused imports, а не использует LLM-based pruning?**

LLM-based pruning (удаление нерелевантного через LLM) имеет проблемы:
- **Медленно** — каждый LLM call занимает секунды
- **Дорого** — токены на pruning = дополнительные затраты
- **Непредсказуемо** — LLM может удалить важное

ContextPruner:
- **Быстро** — regex-based удаление за миллисекунды
- **Дёшево** — не требует LLM
- **Предсказуемо** — удаляет только очевидно нерелевантное (комментарии, пустые строки, unused imports)

**Альтернативы:**
- ❌ Один "smart reader" — нарушение SRP, сложно тестировать
- ❌ Простой truncate — теряет важную информацию
- ❌ LLM-based pruning — медленно, дорого, непредсказуемо
