...
**Почему именно эти три верификатора (Test/Build/Lint), а не больше?**

Три верификатора покрывают 95% сценариев верификации:
- **TestRunner** — проверка корректности логики (функциональные тесты)
- **BuildVerifier** — проверка компиляции/сборки (синтаксическая корректность)
- **LintVerifier** — проверка стиля кода (соглашения проекта)

Другие верификаторы (type checking, security scanning, performance testing):
- **Type checking** — часто часть BuildVerifier (tsc --noEmit для TypeScript)
- **Security scanning** — специализированный инструмент, не для каждого проекта
- **Performance testing** — требует бенчмарков, не для каждой задачи

Эти три верификатора — необходимый минимум для production-ready агента.

**Почему TestRunner auto-detect test framework, а не пользователь указывает?**

Auto-detect (pytest для Python, jest для TypeScript, go test для Go):
- **Удобство** — пользователь не должен указывать фреймворк
- **Автоматизация** — агент сам определяет, как запускать тесты
- **Согласованность** — использует те же команды, что и разработчики

Пользователь может переопределить через конфиг, но по умолчанию auto-detect работает для 90% проектов.

**Почему BuildVerifier — отдельный от TestRunner?**

BuildVerifier и TestRunner решают разные задачи:
- **BuildVerifier** — проверка компиляции/сборки (синтаксис, импорты, типы)
- **TestRunner** — проверка логики (функциональные тесты)

BuildVerifier:
- Быстрее, чем тесты (только компиляция)
- Ловит синтаксические ошибки до запуска тестов
- Работает даже если тесты не написаны

Разделение позволяет:
- Запускать BuildVerifier перед TestRunner (fail fast)
- Пропускать TestRunner, если BuildVerifier не прошёл
- Тестировать независимо

**Почему LintVerifier — отдельный от BuildVerifier?**

LintVerifier проверяет стиль кода, а не компиляцию:
- **BuildVerifier** — код компилируется? (синтаксис, импорты, типы)
- **LintVerifier** — код соответствует соглашениям? (стиль, форматирование, best practices)

LintVerifier:
- Не блокирует компиляцию (код может компилироваться, но не соответствовать стилю)
- Проверяет project-specific соглашения (например, "используем double quotes")
- Может auto-fix некоторые проблемы

Разделение позволяет:
- Запускать LintVerifier после BuildVerifier (сначала компиляция, потом стиль)
- Пропускать LintVerifier для прототипов
- Настраивать правила линтера независимо

**Альтернативы:**
- ❌ Один универсальный верификатор — нарушение SRP, сложно настраивать
- ❌ Только TestRunner — не ловит синтаксические ошибки до запуска тестов
- ❌ Только BuildVerifier — не проверяет стиль кода
- ❌ Больше верификаторов (type checking, security) — избыточно для MVP
