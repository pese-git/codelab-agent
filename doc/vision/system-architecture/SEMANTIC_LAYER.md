...
**Почему vector embeddings для кода, а не только keyword search?**

Keyword search (git grep, ripgrep) имеет ограничения:
- **Точное совпадение** — не находит семантически похожий код (например, "validate email" vs "check email format")
- **Нет понимания контекста** — не различает "bank" (финансовый) и "bank" (река)
- **Зависимость от формулировки** — если пользователь написал "почини баг с авторизацией", а код содержит "fix login issue", keyword search не найдёт

Vector embeddings:
- **Семантическое понимание** — находит семантически похожий код, даже если слова разные
- **Контекстное понимание** — различает разные значения одного слова
- **Независимость от формулировки** — находит релевантный код для разных формулировок

**Почему FAISS, а не другие vector databases?**

FAISS (Facebook AI Similarity Search):
- **Быстрый** — оптимизирован для больших объёмов данных
- **Лёгкий** — не требует отдельного сервера, работает in-process
- **Гибкий** — поддерживает разные метрики расстояния (cosine, L2, dot product)
- **Open source** — свободная лицензия

Альтернативы:
- ❌ Pinecone/Weaviate/Chroma — требуют отдельного сервера, избыточны для MVP
- ❌ pgvector — требует PostgreSQL, усложняет деплой
- ❌ Собственная реализация — медленно, неоптимально

FAISS — оптимальный выбор для coding agent: быстрый, лёгкий, работает in-process.

**Почему chunking strategy: 500 tokens с 50 overlap?**

Chunking (разбиение кода на фрагменты) требует баланса:
- **Слишком маленькие chunks** — теряют контекст (функция разбита на части)
- **Слишком большие chunks** — включают нерелевантный код, занимают много токенов
- **Overlap** — предотвращает разрыв важных конструкций на границе chunks

500 tokens:
- Достаточно для понимания функции/класса
- Не слишком много для LLM context window
- Примерно 2000 символов кода (средняя функция)

50 tokens overlap (10%):
- Предотвращает разрыв на границе chunks
- Не слишком много дублирования
- Сохраняет контекст между chunks

Эти значения подобраны эмпирически и могут быть настроены.

**Почему text-embedding-3-small, а не другие embedding models?**

text-embedding-3-small (OpenAI):
- **Дешёвый** — $0.02 за 1M токенов
- **Быстрый** — низкая латентность
- **Качественный** — хорошо работает для кода
- **Доступный** — не требует установки локальных моделей

Альтернативы:
- ❌ text-embedding-3-large — дороже, незначительно лучше для кода
- ❌ code-embedding (специализированные) — менее доступны, требуют установки
- ❌ Локальные модели (sentence-transformers) — требуют GPU для скорости

text-embedding-3-small — оптимальный баланс цены, скорости и качества.

**Почему RAGProvider — отдельный от VectorIndex?**

Разделение ответственности:
- **VectorIndex** — хранение и поиск векторов (low-level)
- **RAGProvider** — интеграция с LLM (high-level)

RAGProvider:
- Берёт результаты поиска из VectorIndex
- Форматирует их для LLM (контекст + вопрос)
- Управляет prompt engineering для RAG
...
- Тестировать VectorIndex независимо от LLM
- Переиспользовать VectorIndex для других задач (не только RAG)
- Изменять RAG-логику без изменения индексации

**Почему SemanticSearch — hybrid (text + semantic), а не pure semantic?**

Pure semantic search имеет проблемы:
- **Неточность для точных запросов** — если пользователь ищет "calculateTotal", semantic search может вернуть "computeSum" (семантически похоже, но не то)
- **Медленнее** — требует вычисления embeddings для каждого запроса

Hybrid search (text + semantic):
- **Text search** — для точных совпадений (быстро, точно)
- **Semantic search** — для семантического понимания (медленнее, но находит похожее)
- **Ranking** — комбинация результатов с весами

Результат:
- Точные совпадения находят быстро
- Семантически похожий код находит через embeddings
- Лучшее из обоих миров

**Альтернативы:**
- ❌ Только keyword search — не понимает семантику
- ❌ Только semantic search — неточен для точных запросов
- ❌ Другие vector databases — требуют отдельного сервера
