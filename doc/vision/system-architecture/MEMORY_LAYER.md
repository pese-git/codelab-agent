...
**Почему три типа памяти (Task/Session/Project), а не одна unified?**

Unified memory (одна большая память) имеет проблемы:
- **Разные временные масштабы** — задачи живут дни/недели, сессии — часы, проект — месяцы/годы
- **Разная гранулярность** — задачи конкретны (что делали), проект абстрактен (паттерны, зависимости)
- **Разные сценарии использования** — поиск похожих задач vs понимание структуры проекта

Три типа памяти:
- **TaskMemory** — конкретные задачи (что делали, какие решения, какие файлы)
- **SessionMemory** — контекст текущей сессии (открытые файлы, принятые решения, ошибки)
- **ProjectMemory** — долгосрочная память проекта (структура, паттерны, часто используемые файлы)

Каждый тип оптимизирован под свой сценарий:
- TaskMemory — поиск похожих задач для переиспользования решений
- SessionMemory — сохранение контекста между turns в рамках одной сессии
- ProjectMemory — быстрое понимание структуры проекта

**Почему именно такая модель данных для TaskMemory (TaskRecord)?**

TaskRecord содержит:
- `task_description` — описание задачи (для поиска похожих)
- `status` — статус задачи (pending, in_progress, completed, failed)
- `files_modified` — изменённые файлы (для понимания scope)
- `decisions` — принятые решения (для переиспользования)
- `errors` — ошибки и их исправления (для избежания повторения)

Эта модель покрывает 90% сценариев:
- Поиск похожих задач по описанию
- Понимание, какие файлы были изменены
- Переиспользование принятых решений
- Избежание повторения ошибок

**Почему SessionMemory хранит files_opened, decisions, errors?**

SessionMemory — контекст текущей сессии:
- `files_opened` — какие файлы открывали (для понимания focus)
- `files_modified` — какие файлы изменяли (для понимания progress)
- `decisions` — принятые решения (для консистентности)
- `errors` — ошибки и их исправления (для избежания повторения)

Это даёт LLM понимание:
- Над чем работает пользователь
- Что уже сделано в этой сессии
- Какие решения были приняты
- Какие ошибки были исправлены

**Почему ProjectMemory хранит common_patterns и frequently_used_files?**

ProjectMemory — долгосрочная память проекта:
- `common_patterns` — часто используемые паттерны (например, "используем DTO для валидации")
- `frequently_used_files` — часто используемые файлы (для приоритизации)
- `project_conventions` — соглашения проекта (например, "используем async/await")

Это даёт LLM:
- Понимание принятых в проекте паттернов
- Приоритизацию часто используемых файлов
- Соблюдение соглашений проекта

**Альтернативы:**
- ❌ Одна unified memory — разные временные масштабы, сложно оптимизировать
- ❌ Только TaskMemory — нет понимания контекста сессии и проекта
- ❌ Только ProjectMemory — нет памяти о конкретных задачах
