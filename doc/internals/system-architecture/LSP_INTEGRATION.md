...
**Почему LSP protocol, а не custom parsing?**

Custom parsing (regex, AST) имеет ограничения:
- **Неточность** — regex не понимает контекст (перегрузка функций, наследование)
- **Неполнота** — AST не всегда доступен (бинарные зависимости, внешние библиотеки)
- **Сложность** — нужно писать парсер для каждого языка

LSP (Language Server Protocol):
- **Точность** — language server понимает семантику языка (перегрузки, наследование, типы)
- **Универсальность** — один протокол для всех языков (TypeScript, Python, Go, Rust, etc.)
- **Стандарт** — поддерживается всеми major IDE (VS Code, JetBrains, Neovim)

LSP даёт:
- **Go-to-definition** — точное определение символа (даже для перегруженных функций)
- **Find references** — все использования символа (включая наследование)
- **Rename symbol** — безопасное переименование во всём проекте
- **Hover information** — типы, документация, сигнатуры

**Почему LSP — Level 5 (Ultimate), а не раньше?**

LSP требует:
- **Установки language servers** — для каждого языка нужен отдельный server
- **Управления процессами** — нужно запускать, мониторить, перезапускать language servers
- **Ресурсов** — language servers потребляют память и CPU
- **Сложности интеграции** — нужно обрабатывать JSON-RPC, lifecycle, errors

Для MVP (Level 1-3) достаточно:
- Regex-based parsing для базовых случаев
- git grep для поиска ссылок
- Text-based rename (с ограничениями)

LSP — это production-ready решение для Level 5, когда нужна максимальная точность.

**Почему DefinitionResolver — отдельный от LSPClient?**

Разделение ответственности:
- **LSPClient** — низкоуровневый клиент LSP протокола (JSON-RPC, lifecycle)
- **DefinitionResolver** — высокоуровневый компонент (go-to-definition логика)

DefinitionResolver:
- Использует LSPClient для получения данных
- Обрабатывает результаты (multiple definitions, overloads)
- Возвращает удобную структуру (Location)

LSPClient:
- Управляет connection с language server
- Отправляет JSON-RPC запросы
- Получает JSON-RPC ответы

Разделение позволяет:
- Тестировать DefinitionResolver без LSP (mock LSPClient)
- Переиспользовать LSPClient для других компонентов (ReferenceResolver, RenameEngine)
- Изменять логику резолвинга без изменения протокола

**Почему ReferenceResolver использует LSP, а не git grep?**

git grep для поиска ссылок имеет проблемы:
- **Неточность** — находит все вхождения строки, а не символ (например, "bank" в "bank account" и "river bank")
- **Нет понимания контекста** — не различает использование символа и упоминание в комментарии
- **Не работает для перегрузок** — не различает разные overload'ы одной функции

LSP ReferenceResolver:
- **Точность** — находит только использования символа (не комментарии, не строки)
- **Понимание контекста** — различает разные символы с одинаковым именем
- **Работает для перегрузок** — находит использования конкретного overload'а

**Почему RenameEngine — safe rename, а не text replace?**

Text replace (sed, regex) имеет проблемы:
- **Небезопасно** — заменяет все вхождения строки (включая комментарии, строки)
- **Не понимает контекст** — может сломать код (заменить "bank" в "bank account" и "river bank")
- **Не работает для перегрузок** — заменяет все overload'ы

LSP RenameEngine:
- **Безопасно** — заменяет только символ (не комментарии, не строки)
- **Понимает контекст** — заменяет только конкретный символ (не другие с таким же именем)
- **Работает для перегрузок** — заменяет только конкретный overload

Результат: безопасное переименование во всём проекте без поломки кода.

**Альтернативы:**
- ❌ Custom parsing (regex, AST) — неточно, сложно для каждого языка
- ❌ Text-based rename — небезопасно, ломает код
- ❌ git grep для references — неточно, не понимает контекст
