...
**Почему собственный discovery, а не ctags/cscope/existing tools?**

Существующие инструменты (ctags, cscope, LSP) имеют ограничения:
- **ctags/cscope** — устаревшие инструменты, не поддерживают современные языки (TypeScript, Go)
- **LSP** — требует запуска language server для каждого языка, избыточен для discovery
- **tree-sitter** — даёт AST, но не определяет фреймворки, package managers, entry points

Собственный ProjectDiscovery:
- **Универсальность** — работает для всех языков через file signatures (package.json → Node.js, pyproject.toml → Python)
- **Лёгкость** — не требует внешних зависимостей
- **Расширяемость** — легко добавить новые языки и фреймворки

**Почему SearchEngine — abstraction layer над git grep/rg/grep?**

Прямое использование git grep/rg/grep имеет проблемы:
- **Разные интерфейсы** — каждый инструмент имеет свой формат вывода
- **Нет fallback** — если git grep недоступен, нужно вручную переключаться
- **Нет фильтрации** — нужно вручную фильтровать результаты (исключать node_modules, .git)

SearchEngine:
- **Единый интерфейс** — один метод search() для всех инструментов
- **Автоматический fallback** — git grep → rg → grep
- **Встроенная фильтрация** — исключает бинарные файлы, node_modules, .git
- **Рейтинг результатов** — приоритизация по релевантности

**Почему language detection через file signatures, а не content-based?**

Content-based detection (анализ содержимого файлов) имеет проблемы:
- **Медленно** — нужно читать файлы для определения языка
- **Неточно** — файлы могут содержать код на разных языках
- **Сложно** — требует эвристики для определения основного языка

File signatures (package.json → Node.js, pyproject.toml → Python):
- **Быстро** — проверка наличия файла за миллисекунды
- **Точно** — конфигурационные файлы однозначно определяют язык
- **Просто** — не требует анализа содержимого

**Альтернативы:**
- ❌ ctags/cscope — устаревшие, не поддерживают современные языки
- ❌ LSP для discovery — избыточен, требует запуска language server
- ❌ Content-based detection — медленно, неточно
