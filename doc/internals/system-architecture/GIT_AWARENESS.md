...
**Почему включать git state в контекст LLM?**

Git state даёт LLM критически важную информацию:
- **Текущая ветка** — понимание, в каком контексте работает агент (feature branch vs main)
- **Последние коммиты** — понимание недавних изменений (что уже сделано)
- **Изменённые файлы** — понимание, над чем работает пользователь прямо сейчас
- **Staged files** — понимание, что будет в следующем коммите

Без git state:
- LLM не понимает контекст работы пользователя
- Может предложить изменения, которые конфликтуют с недавними коммитами
- Не знает, какие файлы уже изменены

**Почему GitDiffAnalyzer — отдельный компонент, а не часть GitContextSource?**

Разделение ответственности:
- **GitContextSource** — предоставляет текущее состояние Git (snapshot)
- **GitDiffAnalyzer** — анализирует изменения (diff между состояниями)

GitDiffAnalyzer решает другую задачу:
- Сравнение working tree с HEAD
- Сравнение staged changes с working tree
- Сравнение между коммитами/ветками
- Анализ изменений по файлам

Объединение в один компонент нарушило бы SRP.

**Почему GitStatusProvider — отдельный от GitContextSource?**

GitStatusProvider — низкоуровневый компонент:
- Парсит вывод `git status`
- Возвращает списки файлов (modified, untracked, staged, deleted, renamed)

GitContextSource — высокоуровневый компонент:
- Использует GitStatusProvider для получения данных
- Добавляет дополнительную информацию (branch, recent commits)
- Рендерит контекст для LLM

Разделение позволяет:
- Тестировать GitStatusProvider независимо
- Переиспользовать GitStatusProvider в других компонентах
- Изменять формат вывода GitContextSource без изменения парсинга

**Почему git awareness — отдельный слой, а не часть ContextGatherer?**

Git awareness — это не просто поиск файлов:
- Требует понимания Git-концепций (branch, commit, staging)
- Имеет собственную логику (diff analysis, status parsing)
- Интегрируется с ContextRegistry как отдельный source

Выделение в отдельный слой:
- Чёткое разделение ответственности
- Легко тестировать независимо
- Можно отключить для проектов без Git

**Альтернативы:**
- ❌ Не включать git state — LLM не понимает контекст работы
