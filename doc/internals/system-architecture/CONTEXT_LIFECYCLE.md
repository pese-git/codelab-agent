...
**Почему Context Lifecycle — отдельный слой, а не часть ContextManager?**

Context Lifecycle решает принципиально другую задачу, чем ContextManager:
- **ContextManager** — сбор контекста для LLM (task analysis, context gathering, budgeting)
- **Context Lifecycle** — управление жизненным циклом контекста (регистрация источников, отслеживание изменений, immutable baseline)

Разделение ответственности:
- ContextManager может менять реализацию (сегодня git grep, завтра tree-sitter) без изменения Lifecycle
- Context Lifecycle может эволюционировать (добавить новые sources) без изменения ContextManager
- Тестируемость: каждый слой тестируется независимо

**Почему Epoch + Snapshot + Reconciliation, а не просто immutable context?**

Immutable context (полный пересоздание на каждый LLM call) имеет проблемы:
- **Производительность:** пересоздание всего контекста (чтение всех файлов, парсинг) занимает секунды
- **Кэширование:** LLM provider не может кэшировать baseline, если он меняется на каждый call
- **Консистентность:** в рамках одного диалога LLM должен работать с консистентным baseline

Epoch + Snapshot + Reconciliation решает эти проблемы:
- **Epoch** — immutable baseline, который кэшируется provider'ом
- **Snapshot** — отслеживание изменений в sources (что изменилось с последнего epoch)
- **Reconciliation** — инкрементальные updates вместо полного пересоздания

Результат:
- Кэширование baseline (экономия токенов и времени)
- Быстрые updates (только изменения, а не весь контекст)
- Консистентность в рамках epoch

**Альтернативы:**
- ❌ Полный пересоздание контекста на каждый call — медленно, нет кэширования
- ❌ Только immutable context без updates — не видит изменений в проекте
- ❌ Mutable context (всё меняется) — нет консистентности, сложно отлаживать
...
**Почему registry pattern, а не прямые ссылки?**

Прямые ссылки (ContextManager → InstructionContextSource, ContextManager → ProjectContextSource, ...) имеют проблемы:
- **Жёсткая связь:** ContextManager знает о всех конкретных sources
- **Сложность расширения:** добавление нового source требует изменения ContextManager
- **Тестируемость:** сложно mock'ать отдельные sources

Registry pattern решает эти проблемы:
- **Слабая связь:** ContextManager знает только об интерфейсе ContextSource
- **Легко расширять:** добавление нового source = регистрация в registry
- **Тестируемость:** можно mock'ать отдельные sources или весь registry

**Почему render_baseline() и render_updates() — отдельные методы?**

- **render_baseline()** — полный контекст для начала epoch (один раз на сессию)
- **render_updates()** — инкрементальные изменения (mid-conversation messages)

Разделение позволяет:
- Кэшировать baseline (не пересоздавать на каждый call)
- Применять только изменения (экономия токенов)
- Чётко разделять immutable baseline и mutable updates

**Альтернативы:**
- ❌ Прямые ссылки — жёсткая связь, сложно расширять
- ❌ Единый метод render() — нет разделения на baseline и updates
- ❌ Event-based подход (pub/sub) — избыточен для MVP
...
**Почему generic typed sources с Codec?**

Каждый источник контекста имеет свою структуру данных:
- `InstructionContextSource` — `list[InstructionFile]`
- `ProjectContextSource` — `ProjectMetadata`
- `EnvironmentContextSource` — `EnvironmentConfig`

Generic typed sources (`ContextSource[T]`) дают:
- **Типобезопасность:** компилятор проверяет, что loader возвращает правильный тип
- **Код повторного использования:** один базовый класс для всех sources
- **Codec для сравнения:** каждый тип знает, как сравнивать свои значения

**Почему Codec — отдельный класс, а не метод equals?**

Codec инкапсулирует логику сравнения:
- Для простых типов (str, int) — прямое сравнение
- Для сложных типов (list[InstructionFile]) — сравнение по полям
- Для custom типов — можно реализовать custom логику (например, сравнение по hash)

Разделение ответственности:
- ContextSource — загрузка и рендеринг
- Codec — сравнение значений

**Альтернативы:**
- ❌ Нетипизированные sources (ContextSource без generic) — потеря типобезопасности
- ❌ Каждый source реализует свой интерфейс — дублирование кода
- ❌ Сравнение через hash — не работает для сложных типов
...
**Почему change detection через сравнение значений, а не timestamps?**

Timestamps (mtime файлов) имеют проблемы:
- **Неточность:** mtime может не измениться, если файл перезаписан с тем же содержимым
- **Ложные срабатывания:** mtime меняется при каждом сохранении, даже если содержимое не изменилось
- **Не работает для всех sources:** некоторые sources (например, Git status) не имеют mtime

Сравнение значений через Codec:
- **Точность:** определяет реальное изменение содержимого
- **Гибкость:** каждый source определяет свою логику сравнения
- **Универсальность:** работает для всех типов sources

**Почему snapshot хранит последние известные значения?**

Snapshot — это "снимок" состояния всех sources на момент начала epoch. Он нужен для:
- **Обнаружения изменений:** сравнение текущего значения с snapshot
- **Восстановления состояния:** если нужно откатиться к предыдущему epoch
- **Аудита:** понимание, какие значения были в определённый момент времени

**Альтернативы:**
- ❌ Timestamps (mtime) — неточно, не работает для всех sources
- ❌ Hash каждого значения — требует дополнительных вычислений
- ❌ Не хранить snapshot — невозможно обнаружить изменения
...
**Почему incremental updates, а не full rebuild?**

Full rebuild (полный пересоздание контекста на каждое изменение) имеет проблемы:
- **Производительность:** чтение всех файлов, парсинг, рендеринг занимает секунды
- **Токены:** каждый rebuild отправляет весь контекст в LLM (даже неизменённые части)
- **Кэширование:** LLM provider не может кэшировать baseline, если он постоянно меняется

Incremental updates:
- **Быстро:** обновляются только изменённые sources
- **Экономия токенов:** отправляются только изменения, а не весь контекст
- **Кэширование:** baseline остаётся неизменным в рамках epoch

**Почему safe boundary?**

Reconciliation должен происходить в безопасные моменты:
- **Нет pending tool calls** — все инструменты завершили работу
- **User input promoted** — пользовательский ввод обработан
- **Tools settled** — нет ожидающих результатов

Без safe boundary:
- Reconciliation может прервать выполнение инструмента
- Изменения могут быть применены в середине операции
- Состояние может стать несогласованным

**Альтернативы:**
- ❌ Full rebuild на каждое изменение — медленно, дорого
- ❌ Reconciliation в любой момент — несогласованное состояние
- ❌ Не делать reconciliation — контекст устаревает
