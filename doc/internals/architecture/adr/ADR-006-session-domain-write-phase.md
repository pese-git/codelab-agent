...
| 2 | Read-only проекция домена для ядра и executors | `tools/` не затронута, поведение не меняется |
| 3 | Флип source-of-truth в `PromptContext`; `loop.py`, `tool_processor.py` на доменные сеймы | `domain_session` перестаёт быть аддитивным |
