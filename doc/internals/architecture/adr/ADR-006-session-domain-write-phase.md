...
> **Отменено шагом 3a ADR-008 (2026-08-06).** «Постоянная wire-граница» здесь больше не
> действует: журнал стал доменным (`domain/journal.py`), а ACP-нотификация — его проекцией
> (`mapping/journal_mapper.py`). Форму записи знает проекция, а не писатель; набор
> `_REPLAYABLE_UPDATE_TYPES` снят — реплеируемость определяет наличие реплей-формы у
> события. Решение по `session_info_update` (ниже) сохранено без изменений, но выражено
> конструкцией вместо перечисления строк. Форма записи в документе не изменилась.
...
| 3 | Единственный флип: рабочее состояние turn'а на агрегат (`PromptContext`, `loop.py`, `tool_processor.py`, executors) | ✅ 2026-08-03 (см. ниже); `SessionState` больше не носитель внутри turn'а |
| 4 | `session_prompt` под репозиторий; снятие `save_session_merging` и союза `HistoryMessage \| dict` | ✅ 2026-08-03 (см. ниже); слияния нет, форма записи одна |
| 5 | Resume: убрать круг `to_protocol`/`to_domain` в `session_load` | ✅ 2026-08-03 (см. ниже); документ живёт в хранилище, `ignore_imports` пуст |
...
### Фаза D, шаг 3 — флип turn-пути на агрегат (2026-08-03)

Носитель состояния turn'а — доменный `Session`. Wire-документ собирается только на границе
записи; `SessionState` внутри turn-пути не остался нигде. Карта выше отработала как есть:
разведку заново проводить не пришлось, из 140 обращений правки потребовали ровно те 42 сайта с
расхождением вложенности.

**Переведено.** `PromptContext.session` — агрегат, отдельное поле `domain_session` исчезло вместе
с dual-carry. Листовые менеджеры (`state_manager`, `turn_lifecycle_manager` — теперь создаёт
доменный `TurnState`, `event_history_writer`, `tool_call_state`, `tool_call_updates`,
`client_requests`, `tool_call_handler`, `permission_manager`, `tool_policy`), стадии (`directives`,
`llm_loop`, `agent_loop/{loop,tool_processor,updates,llm_caller}`), цепочка `tools/` целиком,
`terminal_alias_registry` — адаптер над сеймами шага 2. Граница с ядром — `DomainSessionView`,
строится в `llm_caller` один раз на вызов.

**Корень P1-45 закрыт.** `loop.py` пишет запись «ассистент + запрошенные им tool_calls» доменным
сеймом вместо сырого dict'а. Второй формы одной записи в истории больше нет.

**Найден дефект, который создал сам флип, и это главный результат живого прогона.**
`storage.save_session` инкрементирует ревизию **в переданном объекте**, а turn с этого шага
передаёт одноразовую проекцию агрегата. Ревизия оставалась на значении загрузки, поэтому каждая
следующая запись turn'а считалась конфликтом и уходила в слияние — а правило «`active_turn` из
свежей копии» выбрасывало только что заведённый `permission_request_id`. Ответ на разрешение после
этого не находил сессию, и turn висел навсегда. Тесты этого не показали: они не гоняют цепочку из
двух пауз подряд. Показал живой прогон батча из трёх вызовов — второе разрешение уходило клиенту,
ответ приходил, и сервер молчал.

Лечение — `save_domain_session_merging`: единственная точка, где агрегат превращается в документ,
возвращает агрегату ревизию состоявшейся записи. Это восстанавливает свойство, которое до флипа
держалось само собой (запись мутировала тот же объект), и заодно убирает каскад ложных слияний.
Симптом стоит записать отдельно: **проекция теряет то, что писало хранилище в объект** — любой
следующий шаг, который заменит носитель, обязан проверить этот класс.

**Гейты сняты.**

*Живой прогон `fsReadPath`* (изолированный `CODELAB_HOME`, покадрово): кадр «turn отложен, ответа
ещё нет» — ревизия 2, `active_turn` есть, `phase: waiting_client_rpc`, `pending: fs_read`, статус
вызова `in_progress` (фикс P2-55 держится); после ответа — ревизия 3 (одна запись),
`active_turn: null`, pending снят, на диске `completed`.

*Расхождений wire↔диск нет:* клиенту ушли `completed` по трём вызовам батча, на диске те же три
`completed`.

*Байт-идентичность документа — не достигнута, и достигнута быть не могла.* Сравнение снимков
одного и того же turn'а на `68276f67` и после флипа: всё вне истории совпадает поле в поле, в
истории 6 записей и все значимые поля равны. Отличий два, оба — следствие снятия второй формы
записи: запись «ассистент + tool_calls» получила `timestamp` (раньше писалась сырым dict'ом без
него), и JSON стал показывать явные `null` у полей, которых частичный dict просто не содержал.
Формулировку гейта нужно читать как «ноль потерь и ноль изменённых значений» — в этом виде он
зелёный; буквальная байт-идентичность противоречила бы цели шага.

**Второй дефект флипа нашли не гейты, а лог живой работы — и это важнее самого дефекта.**
`~/.codelab/logs`: 54 подряд `'DomainSessionView' object has no attribute 'config'` на
`fs/read_text_file` и `terminal/create`. Причина: цепочку `tools/` зовут **два** вызывающих —
turn-путь (агрегат) и ядро/Context Manager, которое держит read-проекцию порта и передаёт её в
`ToolRegistry.execute_tool` как есть. До флипа оба работали случайно: и `SessionState`, и
проекция отвечали на `session.cwd` структурно. После — только агрегат. Прод-эффект: Context
Manager не собирал контекст вообще, деградируя молча (горячий путь не падает по правилу).

Лечение — разворачивание проекции в агрегат на границе `tools/`, в единственной точке
(`SimpleToolRegistry.execute_tool`); проекция для ядра остаётся read-only. Знание о проекции не
расползается по исполнителям.

Чего не хватило гейтам: все три смотрели на **документ сессии**, а сломанной оказалась ветка,
которая документ не пишет. Правило на будущее — при смене носителя перечислять *всех* вызывающих
общей цепочки, а не только тех, кто пишет состояние: у `tools/` их два, и второй виден только под
включённым Context Manager. Регресс закрыт тестами
(`tests/server/tools/test_registry_unwraps_session_view.py`), включая «мутация сквозь проекцию
доходит до сессии» — иначе разворачивание могло бы вернуть копию и дать тихое «успешно ничего не
изменил».

**Что живой лог подтвердил помимо этого** (сессия `sess_f1f6e956e943`, 171 ревизия, 69 вызовов):
ноль расхождений «последний статус в `events_history` ↔ статус в `tool_calls`», все 69 вызовов
имеют ответ `role: tool`, дублей ответов нет, все 69 записей «ассистент + tool_calls» несут
`timestamp` — то есть вторая форма записи в проде действительно исчезла.

**Побочно замечено, не входит в шаг:** после нормального завершения turn'а `active_turn` остаётся
на диске с `phase: awaiting_permission`. Присутствует **в обоих** снимках, то есть
предсуществующий; ведётся отдельно. Двойной спавн сервера (пары pid в одном миллисекундном окне) и
stdout litellm в логе — прежние находки гигиены логов, к флипу отношения не имеют.

**Тесты.** Turn-путь строит агрегат через `tests/server/_domain_sessions`: `make_domain_session`
идёт через маппер от wire-формы — тем же путём, каким сессию получает прод, поэтому расхождение
маппера видно тестам сразу; `wire_history` сверяет историю с тем, что уезжает на диск. Граница
«агрегат там, где turn, wire там, где диск» проведена по месту: фикстуры, сеющие документ в
хранилище, остались wire намеренно. Обёртка `session/prompt` в тест-фабрике делегирует
прод-обработчику — по той же причине, что и отмена: копия тела разошлась бы по типу носителя.

**Снято из `ignore_imports`** (import-linter): протечки `protocol.state` в
`agent.context.file_cache_decorator` и `tools.executors.decorators.base`. Остаток —
`storage.base`, и это не протечка: форма документа — дом хранилища. Ребро уйдёт с шагом 5.

**Что осталось в фазе D:** шаг 4 (`session_prompt` под репозиторий, снятие `save_session_merging`
и союза `HistoryMessage | dict`) и шаг 5 (resume без круга `to_protocol`/`to_domain`). Слияние
после этого шага стало **тише**, но не исчезло: `persist` по-прежнему пишет вне транзакции, и
`save_domain_session_merging` — та самая «защита, а не цель» из шапки `session_merge`.

### Фаза D, шаг 4 — turn пишет командами; слияние и союз сняты (2026-08-03)

Носителем записи turn перестал быть его копией. Каждое изменение состояния применяется к
агрегату, загруженному **в момент применения**, под блокировкой сессии, и коммитится тут же —
форма, зафиксированная решением шага 8. Инвариант «turn никогда не записывает документ, который
держал через `await`» стал конструктивным, а не дисциплинарным.

**Шов — `protocol/session_commands.py`.** `apply(command)` открывает транзакцию репозитория,
применяет синхронную команду к свежему агрегату и коммитит. Рабочая копия turn'а остаётся тем же
объектом: состояние переносится в неё поэлементно, поэтому `PromptContext`, `DomainSessionView` и
цепочка `tools/` продолжают читать актуальное, не зная о транзакциях. `require_active_turn` — это
правило «`active_turn` из свежей копии», переехавшее в guard применения; «отмена главнее» уже
жила в `ToolCallRegistry.update_status`; «журналы по общему префиксу» стало нечем выражать —
запись добавляется в свежий журнал.

**Гранулярность — команда, а не мутация.** Дословное «одна мутация = одна транзакция» разорвало
бы неделимые решения: приём промпта (заголовок + запись пользователя + событие реплея), итог
вызова (содержимое + статус + ответ модели), пауза на разрешение (`permission_request_id` +
фаза). Разрыв внутри них даёт наблюдаемое полусостояние — вызов без `role: tool`, разрешение без
идентификатора, по которому его найдёт входящий ответ. Поэтому команда = одно решение, и это
единица атомарности.

**Шаг создал дефект того же класса, что чинил, и поймал его собственным гейтом.** Первая версия
`apply` переносила состояние в рабочую копию **внутри** области транзакции — а `updated_at` и
ревизию хранилище штампует на коммите, то есть на выходе из неё. Копия оставалась на значениях
загрузки: то же «проекция теряет то, что писало хранилище», что дало зависший turn на шаге 3, и
вдобавок устаревший `updatedAt` клиенту. Перенос сдвинут за выход из области; гейт «одна
команда — одна ревизия, и рабочая копия знает состоявшуюся» держит это явно.
`SessionRepository.save_session` теперь возвращает агрегату и ревизию: без слияния
несинхронизированная ревизия перестаёт быть лишним слиянием и становится жёстким отказом CAS.

**Два временных шва, помеченных как временные.** Исполнители инструментов (реестр терминалов,
`set_config_value`) и slash-команды пишут в рабочую копию по ходу собственного `await`; своего
шва команд у них нет, потому что цепочку `tools/` зовут два вызывающих (turn и Context Manager,
шаг 3), и порт исполнителя пришлось бы менять для обоих. Их решения переносятся явным
перечислением — `SessionCommands.carry_working_changes`. Перечень конечный и виден целиком;
мутация исполнителя, в него не попавшая, до диска не доедет. Шов уходит, когда эти пути получат
собственные команды.

**Слияние удалено целиком, союз снят.** `session_merge` больше не существует — гейт требовал
удаления, а не мёртвого модуля. Союз `HistoryMessage | dict` держался в трёх местах
(`SessionState.history`, сборка истории в `to_protocol`, сигнатура `HistoryMapper.to_domain`) и
означал две формы одной записи; это и есть корень P1-45, а `_canonical_entry` был заплаткой
поверх. Последним писателем сырого dict'а оставался `session/load` через `answer_deferred_batch`:
wire-сеймы `SessionState` переведены на модель. Документы прошлых версий приводятся к
`HistoryMessage` валидацией при загрузке (`extra="allow"` сохраняет `tool_calls`) — проверено на
живом документе в 84 записи, потерь нет.

**Изменение контракта.** Сбой записи сессии больше не проглатывается. Раньше финальный `save` в
`finally` логировал ошибку, и turn отвечал клиенту по состоянию, которого на диске нет; второй
шанс давало финальное сохранение. Финального сохранения нет — несостоявшаяся команда видна
вызывающему.

**Гейты сняты.**

*Юнит, на `JsonFileStorage`, не на памяти* (`tests/server/protocol/test_step_persistence.py`):
каждый обработанный вызов виден на диске; отложенный хвост батча на диске **до** паузы; сбой
записи виден вызывающему; одна команда — одна ревизия, и рабочая копия знает состоявшуюся. На
in-memory backend класс «мутировали и не сохранили» невидим (P1-49), поэтому гейты держатся на
файловом.

*Живьём, два прогона на собранной из дерева сборке* (`sess_26ac5ac31a8f`, 40 вызовов;
`sess_bde8d24c7e15`, 34 вызова — второй уже и со снятым союзом): 32 и 29 подряд идущих цепочек
«пауза на разрешение → ответ клиента → возобновление» без единого обрыва. Это ровно та цепочка из
двух пауз подряд, что валила turn на шаге 3 и которой нет в юнит-тестах. Слияний — ноль (модуля
нет), пропущенных команд (`session_gone`/`turn_closed`) — ноль, ошибок — ноль. Инварианты
документа: все вызовы отвечены `role: tool`, дублей нет, расхождений «последний статус в
`events_history` ↔ статус в `tool_calls`» нет, все записи «ассистент + tool_calls» с `timestamp`.
На втором прогоне все 53 записи истории на диске несут один набор полей — вторая форма исчезла
наблюдаемо, а не только по типам.

**Цена измерена на живом документе и оказалась выше оценки.** Оценка шага 8 («~15 команд на turn,
≈65 мс») считала команды на turn, а turn содержит десятки вызовов: фактически ~8 команд на вызов
инструмента — 309 ревизий на 34 вызова при документе 347 КБ, то есть около 107 МБ записи за
трёхминутную сессию (второй прогон: 329 ревизий, 485 КБ). По времени незаметно — латентность
съедает модель, — но линейность по размеру документа перестала быть теорией. Это ровно последняя
строка «Порядка работ» ADR-007 (расщепить горячее состояние и холодный лог), и после этого шага
она становится следующей по ценности.

**Тесты.** Turn-путь переведён на настоящие доменные сессии: мок нельзя подать в команду — она
обязана ходить в хранилище, поэтому «команда не доехала» на моке было неотличимо от «команда
применилась». Это тот же слепой участок, что скрывал read-проекцию в цепочке `tools/` на шаге 3.
Переход сразу же поймал три места, где тесты подсовывали `MagicMock` как результат исполнения
инструмента: объект мока доезжал до маппера истории. Фикстуры, клавшие в историю сырые dict'ы,
переведены на модель — pydantic не валидирует `append` и присваивание, поэтому такие записи молча
сохраняли старую форму; прогон с `-W error::UserWarning` теперь чист.

**Что этот шаг не сделал, хотя рядом.** Хвост `active_turn` на диске (`phase:
awaiting_permission` при пустых идентификаторах) — предсуществующий, но теперь диагностирован
точно: `phase` никогда не сбрасывается (`TurnLifecycleManager.set_turn_phase` — мёртвый код, ноль
вызывающих), а turn, завершившийся на resume-пути, не проходит через `TurnLifecycleStage(close)`
и не закрывается. Подчищается командой «снять stale active_turn» на следующем `session/prompt`.

**Что осталось в фазе D:** шаг 5 — resume без круга `to_protocol`/`to_domain`, после которого
`SessionState` живёт только внутри репозитория и уходит последняя строка `ignore_imports`
(`storage.base`).

### Фаза D, шаг 5 — resume на агрегате, документ уехал в хранилище (2026-08-03)

Шаг закрыт тремя проходами; каждый снимал свою причину, по которой `SessionState` не мог остаться
только внутри репозитория.

**5a. Загрузка была кругом.** Репозиторий отдавал агрегат, команда переводила его в документ,
реплей и очистка правили документ, и он же переводился обратно ради записи. Круг существовал не по
необходимости, а потому что реплей писался до фазы D. На домен переведены `SessionReplayer`,
`_cleanup_session_state`, `_replay_tool_calls_fallback`, `_drop_terminals_from_previous_process` и
сам `session_load`; параметр `storage` из него убран — сессию передаёт вызывающий, уже открывший её
(вторая загрузка давала бы вторую копию, P2-42). Контекст подключения принимается сеймом
`apply_client_context`: `SessionConfig` заморожен, и замена конфигурации целиком честнее правки
полей — иначе вызывающий обязан знать, что `cwd`/`mcp_servers`/возможности принадлежат
подключению, а `config_values` и стратегия переживают загрузку.

*Заодно исправлен регресс шага 4.* `ensure_initialized` подрезает `available_commands` при
восстановлении MCP-prompt'ов, а шаг 4 стал передавать ему одноразовую проекцию — правки переставали
доезжать до сессии, и клиент мог получить список команд, которого на диске нет. Тихий регресс,
найденный при переводе загрузки, а не тестом: тесты этого пути подавали проекцию сами.

**5b. MCP-путь держался на буфере в документе.** Обработчики prompt-команд складывались в transient
`SessionState.mcp_prompt_handlers`, копировались в реестр рантайма и поле очищалось. Буфер в
персистентном DTO — не хранилище, а перевалочный пункт, и именно он не давал перевести менеджер на
домен. Обработчики теперь пишутся сразу в реестр рантайма (их дом), регистратор получает приёмник
параметром, поле удалено — после снятия буфера у него не осталось пользователей. Следом менеджер
переведён на агрегат целиком, и три проекции, введённые шагом 4, исчезли вместе с переносом
`available_commands` обратно.

*`session/new` конвертирует документ фабрики в агрегат ДО side effects.* MCP-setup правит
`available_commands` и обязан править тот объект, который будет сохранён; прежний порядок работал
случайно.

**5c. Документ переехал в хранилище и получил честное имя.** `SessionState` → `SessionDocument` в
`storage/document.py` вместе с `ToolCallState`, `ActiveTurnState`, `PendingClientRequestState`,
`ClientRuntimeCapabilities` и всеми миграциями схемы. Переименование сделано тем же проходом
сознательно: «состояние сессии» — это доменный `Session`, а два `state` рядом были главным
источником путаницы всей фазы; отдельный проход по тем же ~100 файлам ничего бы не добавил.
В `protocol/state.py` остался протокольный рантайм (`ProtocolOutcome`, `LLMLoopResult`,
`PromptDirectives`, `ToolResult`, `PendingToolExecution`, `PreparedFsClientRequest`) — он на диск
не уезжает.

*Перенос вскрыл настоящую протечку, которую маскировал игнор.* Миграция документа зовёт
`normalize_mode`, а `mode.py` лежал в `protocol/` — то есть хранилище тянуло протокол не только
типом документа. Словарь режимов переехал в `domain/mode.py`: режим описывает поведение агента в
сессии, а не ACP-обмен. Заодно его логирование переведено на structlog по конвенции проекта, а
тесты — с текста сообщения на имя события.

**Признак готовности снят.** `ignore_imports` контракта «Server layers» пуст: протечек
`protocol.state` в нижние слои не осталось, долг ADR-003 закрыт. Оставшиеся конверсии вне
репозитория — две, обе объяснимы: `session_new` (фабрика собирает документ — заявленная постоянная
wire-граница) и легаси-ветка `permission_response` с уже задокументированным предсуществующим
дефектом.

**Гейты.** `make check` — 7534 теста; import-linter — 4 контракта, 0 нарушений, 0 исключений. Живой
прогон на собранной из дерева сборке: сессия, созданная предыдущей сборкой, прочитана и записана
новой без ошибок валидации — перенос модуля не сломал совместимость на диске; 28 цепочек
разрешений подряд, ноль пропущенных команд, инварианты документа на 34 вызовах целы, форма истории
одна.

**Фаза D завершена.** Носитель состояния — доменный агрегат на всех прикладных путях; запись — только
командами в коротких транзакциях; документ — дело хранилища. Следующее по ценности вне этой фазы —
последняя строка «Порядка работ» ADR-007: расщепить горячее состояние и холодный лог, потому что
каждая команда сегодня переписывает весь документ (измерено: ~300 ревизий и ~100 МБ записи за
трёхминутную сессию).
