...
**Живьём не проверено:** в разобранных прогонах `pending_client_request` не заводился ни разу —
директивы `/tool` в них не было, а terminal-цепочка идёт через `ClientRPCService`. Отмечено прямо:
шаг закрыт тестами, и это единственная транзакция фазы D без живого подтверждения.

**Что осталось в фазе D:** prompt-turn + resume — атомарно, самый крупный шаг. После него
`SessionState` остаётся только внутри репозитория, и снимаются корни P1-45 (союз типов в истории),
слияния как механизма и остатка P2-46 (`phase` без владельца).
