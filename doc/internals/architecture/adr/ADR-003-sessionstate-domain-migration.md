...
> **Закрыт 2026-08-03 фазой D ADR-006.** Долг снят целиком: рабочая модель прикладных путей —
> доменный `Session`, документ сессии переехал в `storage/document.py` и называется
> `SessionDocument`, `ignore_imports` контракта «Server layers» пуст. Текст ниже описывает
> исходное состояние и план миграции и оставлен как история решения; имя `SessionState` в нём
> относится к тому, что сегодня называется `SessionDocument`.
