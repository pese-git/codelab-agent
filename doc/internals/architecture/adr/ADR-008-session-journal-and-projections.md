...
Ожидание приёмки не изменилось: прогон с намеренно оставленным остатком (команда в терминале без
последующего `release` от модели), запись `turn_terminals_released` с `released>0`, `live` → 0 на
границе turn'а, ноль `terminal_alias_not_found` после.
