...
### Шаг 3b: формат хранения стал доменным — ✅ сделано (статические гейты)
...
- **Проверено на настоящем документе с диска** (`sess_2c4726e9007e`, v10, 108 записей): после
  ...
  доменными.
- ⏳ **Живой прогон не проведён.** Шаг не считается закрытым без него. Ожидаемое наблюдение:
  `schema_version: 11` в документе, записи вида `{event, at, data}`, и `session_loaded` с той же
  разницей `events_history` против `history_notifications`, что и до правки.
