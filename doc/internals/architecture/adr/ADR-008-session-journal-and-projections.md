...
Ожидание приёмки не изменилось, но путь сузился: нужен прогон со **штатно завершившимся** turn'ом,
после которого остался дожданный остаток (команда в терминале без `release` от модели). Запись
`turn_terminals_released` с `released>0`, `live` → 0 на границе turn'а, ноль
`terminal_alias_not_found` после.
