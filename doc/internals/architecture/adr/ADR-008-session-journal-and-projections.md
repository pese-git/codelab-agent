...
Ошибок за оба процесса ноль. Ожидание приёмки не изменилось: прогон со **штатно завершившимся**
turn'ом, после которого остался дожданный остаток, — запись `turn_terminals_released` с
`released>0`, `live` → 0 на границе turn'а, ноль `terminal_alias_not_found` после.
...
* **Вторая половина P2-58** (владение освобождением терминалов) — **сделана** в шаге 5.3: остаток
  освобождает агент на трёх путях, где turn кончается по воле модели, и только дожданный. Ждёт
  приёмки живьём на прогоне с намеренно оставленным остатком. Разбор — в «Ходе реализации».
