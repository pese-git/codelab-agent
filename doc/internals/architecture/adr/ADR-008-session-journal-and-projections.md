...
- ⏳ **Живой прогон не проведён.** Сценарий двух одновременных разрешений сам по себе редок: за
  два прогона он случился один раз, а батч его не даёт — на первом же требующем разрешения
  вызове цикл встаёт, а хвост уходит в `tool_calls_deferred_to_resume`. Приёмку надо ставить
  намеренно.
