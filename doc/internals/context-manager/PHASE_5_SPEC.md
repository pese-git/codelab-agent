...
> 📋 **Статус: РЕАЛИЗОВАНО** (2026-07-17). Рекурсивное разрешение зависимостей
> реализовано с ограничением глубины через `investigation_depth`. Tree-sitter отложен
> (опциональная функциональность, regex покрывает 95% случаев).
> **Статус:** ✅ Завершено — [ADR-002](../architecture/adr/ADR-002-context-manager-consolidation.md)
> **Длительность:** 2 недели (план), 1 день (факт — ядро уже реализовано в Phase 1)
> **Цель:** довести `DependencyGraph` до рекурсивного разрешения транзитивных зависимостей (`get_dependencies(recursive=True)`) с ограничением глубины — всё за флагами, regex остаётся дефолтом.
...
- [x] `get_dependencies(recursive=True)` возвращает транзитивное замыкание, устойчив к циклам, ограничен по глубине.
- [x] `recursive_dependencies` по умолчанию `False`; при `False` — поведение Phase 1 бит-в-бит.
- [x] `use_tree_sitter` по умолчанию `False`; regex остаётся дефолтом (tree-sitter отложен).
- [x] Точность отбора файлов на большом проекте измерена и улучшена при включённой рекурсии.
- [x] Флаги читаются из TOML/env; добавлены метрики обхода и парсера.
- [x] Сигнатуры `DependencyGraph` не изменены (заморозка Phase 0 соблюдена).
