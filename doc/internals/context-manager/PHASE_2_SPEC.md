...
### T2.7 — Безопасность и жизненный цикл кэша файлов
- `FileContentCache` живёт **только в RAM и только в пределах сессии** (`SessionFileCacheRegistry`); при завершении/закрытии сессии — гарантированная очистка (нет переживания между сессиями).
- Содержимое файлов **не логируется и не попадает в трейсы/метрики** (только пути и размеры) — согласовать с [OBSERVABILITY.md](./OBSERVABILITY.md).
- Кэш не пишется на диск; LRU-предел `cache_max_files` ограничивает объём в памяти.
- Кэш уважает границы доступа ACP `ToolRegistry`: кэшируются только успешно прочитанные через ACP файлы (никакого обхода прав).
- **Acceptance:** после закрытия сессии кэш пуста (тест на очистку); содержимое файлов отсутствует в логах/спанах (тест-проверка); превышение `cache_max_files` вытесняет по LRU, а не растёт неограниченно.
...
- [ ] Кэш очищается при закрытии сессии; содержимого файлов нет в логах/трейсах; LRU-предел соблюдается (T2.7).
