...
- Тяжёлое 3-фазное сжатие (`ensure_context_fits()`) в Phase 1 **не реализуется**: единый компактор собирается в Phase 3 (фаза Skeletonize переиспользует `CodeSkeletonizer` из Phase 2). В Phase 1 — только бюджетное усечение слоя A.
...
- Не реализует инкрементальность — `ContextEpoch`/`ContextSnapshot`/`ContextReconciler`: `baseline` пересобирается каждый ход (Phase 4).
- Не реализует полное 3-фазное сжатие `ensure_context_fits()` (Prune→Skeletonize→Summarize) — собирается в Phase 3; в Phase 1 только бюджетное усечение слоя A.
