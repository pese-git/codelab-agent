...
- **[Context Lifecycle](../system-architecture/CONTEXT_LIFECYCLE.md)** — ContextRegistry, ContextSource, InstructionContextSource, SkillContextSource, ContextSnapshot, ContextEpoch
...
- **[Skills System](../../../openspec/changes/skills-system-support/specs/skills-system/spec.md)** — SkillRegistry, SkillContextSource (каталог skills как источник контекста)
...
Context Manager разделяет систему на три уровня:
...
└─────────────────────────────────────────────────────────────┘
                             ↓
┌─────────────────────────────────────────────────────────────┐
│  Уровень 2: Context tools (видит LLM, высокоуровневые)      │
│                                                              │
│  • context/gather — собрать контекст по теме                │
...
**LLM видит Context tools** — это значит:

1. Agent может формулировать потребности в контексте ("мне нужно понять регистрацию")
2. ContextManager решает КАК удовлетворить потребность (поиск, чтение файлов, граф зависимостей)
3. Разделение ответственности: Agent — ЧТО, ContextManager — КАК
4. Аналогия с ORM: бизнес-логика говорит "дай мне пользователя", а не управляет SQL-кэшем вручную
...
**Agent отвечает за:**
- Формулирование потребности в информации ("Что нужно понять?")
- Принятие решений о том, что исследовать
- Написание кода, исправление ошибок

**ContextManager отвечает за:**
- Управление состоянием контекста (хранение, обновление, сжатие)
- Сбор контекста через ContextGatherer
- Бюджетирование токенов
- Snapshot и Epoch management

**Ключевой принцип:** Agent не управляет ContextManager напрямую. Он формулирует потребность, а ContextManager решает, как её удовлетворить.

### Два режима работы

#### Режим 1: Автоматический сбор (по умолчанию)

Перед каждым LLM call ContextManager автоматически собирает контекст:

```python
# Стратегия вызывает build_context()
context = await context_manager.build_context(session, task)

# ContextManager автоматически:
# 1. TaskAnalyzer — анализирует задачу
# 2. ContextGatherer — ищет релевантные файлы
# 3. DependencyGraph — строит граф зависимостей
# 4. TokenBudgetManager — ограничивает размер
```

#### Режим 2: Явный запрос (опционально)

Agent может явно запросить дополнительный контекст через tool `context/gather`:

```python
# LLM вызывает tool
await context_gather(objective="registration flow")

# ContextManager:
# 1. ContextGatherer — ищет файлы по теме
# 2. Сохраняет результаты в контекст
# 3. Обновляет snapshot
# 4. Применяет budget constraints
```
...
Шаг 1: TaskAnalyzer
  → TaskProfile(feature="email validation")

Шаг 2: ContextManager.build_context() (автоматически)
  → Автоматический сбор начального контекста
  → Поиск файлов: "email", "validation"
  → Чтение: auth.dto.ts, auth.service.ts

Шаг 3: LLM call
  → LLM получает контекст и начинает работать

Шаг 4: LLM понимает: "Мне нужно посмотреть, как работает регистрация"
  → LLM вызывает: context/gather(objective="registration flow")

Шаг 5: ContextGatherer
  → SearchEngine: поиск "registration"
  → DependencyGraph: анализ зависимостей
  → ProjectDiscovery: определение структуры

Шаг 6: ContextManager.store()
  → Добавление результатов в контекст
  → Подсчёт токенов
  → Pruning если нужно

Шаг 7: LLM получает обновлённый контекст
  → CurrentContext(...)
  → Продолжает работу
```

### ContextManager как база данных контекста

Аналогия с ORM: бизнес-логика говорит "дай мне пользователя", а не управляет SQL-кэшем и индексами вручную.

**Методы ContextManager:**
...
class ContextManager:
    ...
    async def query(self, session, objective: str) -> GatherResult:
        ...
        result = await self.gatherer.gather(objective=objective)
    ...
    def store(self, session, result: GatherResult) -> None:
        ...
        session.context.add_files(result.files)
        self.snapshot.capture()
    
    # Компакция (используется стратегиями)
    async def ensure_context_fits(self, history) -> list[LLMMessage]:
        """Сжать историю если превышает лимит."""
        ...
    
    # Расширение (позже)
    async def compact(self, session) -> None:
        """Сжать контекст."""
        ...
    
    async def snapshot(self, session) -> ContextSnapshot:
        """Создать снимок контекста."""
        ...
    
    async def epoch(self, session) -> ContextEpoch:
        """Управлять baseline."""
        ...
```

**Что Agent НЕ знает:**
- Epoch, Snapshot, Budget, Pruning
- Как именно ищутся файлы
- Как строится граф зависимостей
- Как сжимается контекст

**Что Agent знает:**
- "Мне нужен контекст по регистрации"
- "Мне нужно понять, как работает авторизация"
- "Подготовь контекст для изменения DTO"
...
### Диаграммы взаимодействия для каждой стратегии
...
sequenceDiagram
    autonumber
    participant U as User
    participant S as SingleStrategy
    participant CM as ContextManager
    participant TA as TaskAnalyzer
    participant CG as ContextGatherer
    ...
    U->>S: prompt
    S->>CM: build_context(session, task)
    CM->>TA: analyze(task)
    TA-->>CM: TaskProfile
    CM->>CG: gather_context(profile)
    CG->>Tools: git ls-files, git grep, read_file
    Tools-->>CG: файлы и содержимое
    CG-->>CM: GatheredContext
    CM-->>S: list[LLMMessage]

    S->>LLM: context + task
    LLM->>Tools: fs/read, fs/write, terminal
    Tools-->>LLM: результаты
    LLM-->>S: response

    S->>CM: ensure_context_fits(history)
    CM-->>S: сжатая история
    S-->>U: результат
```
...
sequenceDiagram
    ...
    participant CSM as ChildSessionManager

    U->>OS: prompt

    rect rgb(230, 245, 255)
        Note over OS,Orch: Шаг 1: Orchestrator решает кого вызвать
        OS->>CM: build_context(orch_session, task)
        CM-->>OS: list[LLMMessage]
        OS->>Orch: context + task
        Orch-->>OS: RouteDecision(target="architect")
    end

    rect rgb(255, 240, 230)
        Note over OS,Arch: Шаг 2: Architect анализирует задачу
        OS->>CM: build_context(arch_session, payload)
        CM-->>OS: list[LLMMessage]
        OS->>Arch: context + payload
        Arch-->>OS: Plan + Tasks
    end

    rect rgb(230, 255, 230)
        Note over OS,CSM: Шаг 3: Обработка ответа Architect
        OS->>CM: process_subagent_response(arch_response, orch_session)
        CM->>TS: slice(response)
        TS-->>CM: summary
        CM->>CSM: create(orch_session, response)
        CSM-->>CM: child_session (architect)
        CM->>CSM: link(parent, child)
        CM-->>OS: SlicedResult(summary, child_session_id)
    end

    rect rgb(245, 230, 255)
        Note over OS,Coder: Шаг 4: Coder пишет код
        OS->>CM: build_context(coder_session, payload)
        CM-->>OS: list[LLMMessage]
        OS->>Coder: context + payload
        Coder-->>OS: Code changes
    end

    rect rgb(230, 255, 230)
        Note over OS,CSM: Шаг 5: Обработка ответа Coder
        OS->>CM: process_subagent_response(coder_response, orch_session)
        CM->>TS: slice(response)
        TS-->>CM: summary
        CM->>CSM: create(orch_session, response)
        CSM-->>CM: child_session (coder)
        CM-->>OS: SlicedResult(summary, child_session_id)
    end

    OS->>CM: ensure_context_fits(orch_history)
    CM-->>OS: сжатая история
    OS-->>U: результат
```
...
sequenceDiagram
    ...
    rect rgb(230, 245, 255)
        Note over CS,CM: Шаг 1: Сбор общего контекста
        CS->>CM: build_context(session, task)
        CM-->>CS: shared_context
    end

    rect rgb(255, 240, 230)
        Note over CS,A3: Шаг 2: Parallel broadcast всем агентам
        par asyncio.gather
            CS->>A1: shared_context + task
            A1-->>CS: answer_A
        and
            CS->>A2: shared_context + task
            A2-->>CS: answer_B
        and
            CS->>A3: shared_context + task
            A3-->>CS: answer_C
        end
    end

    rect rgb(245, 230, 255)
        Note over CS: Шаг 3: Conflict Resolution
        CS->>CS: Priority Queue → winner = Agent B
    end

    rect rgb(230, 255, 230)
        Note over CS,CSM: Шаг 4: Обработка ТОЛЬКО winner
        CS->>CM: process_subagent_response(winner_B, session)
        CM->>TS: slice(answer_B)
        TS-->>CM: summary
        CM->>CSM: create(session, answer_B)
        CSM-->>CM: child_session (Agent B)
        CM-->>CS: SlicedResult(summary, child_session_id)
    end

    Note over CS: Ответы A и C — только в EventTimeline<br/>Child sessions НЕ создаются
...
sequenceDiagram
    ...
    rect rgb(230, 245, 255)
        Note over HS,P: Шаг 1: Primary получает контекст
        HS->>CM: build_context(primary_session, task)
        CM-->>HS: list[LLMMessage]
        HS->>P: context + task
    end

    alt Primary решает делегировать
        rect rgb(255, 240, 230)
            Note over HS,Sub: Шаг 2: Делегирование Sub-agent
            HS->>HS: TaskInvocation → AgentRequest
            HS->>CM: build_context(child_session, payload)
            CM-->>HS: list[LLMMessage]
            HS->>Sub: context + payload
            Sub-->>HS: TaskResult
        end

        rect rgb(230, 255, 230)
            Note over HS,CSM: Шаг 3: Обработка ответа Sub-agent
            HS->>CM: process_subagent_response(result, primary_session)
            CM->>TS: slice(result)
            TS-->>CM: summary
            CM->>CSM: create(primary_session, result)
            CSM-->>CM: child_session
            CM->>CSM: link(parent, child)
            CM-->>HS: SlicedResult(summary, child_session_id)
        end

        HS->>CM: ensure_context_fits(primary_history)
        CM-->>HS: сжатая история

        HS->>P: summary + обновлённый контекст
        P-->>HS: финальный ответ
    else Primary отвечает сам
        P-->>HS: прямой ответ
    end
...
**Почему LLM-классификация, а не правила/regex?**
Задачи пользователей формулируются на естественном языке с огромным разнообразием: "Добавь валидацию", "Почини баг с авторизацией", "Рефакторни этот модуль". Правила и regex не способны охватить это разнообразие без экспоненциального роста сложности. LLM-классификация даёт гибкость: одна модель покрывает все варианты формулировок.

**Почему именно эти поля TaskProfile?**
- `task_type` — определяет стратегию поиска (для bug fix ищем тесты и логи, для feature — DTO и сервисы)
- `search_terms` — ключевые слова для поиска файлов (извлекаются из контекста задачи)
- `likely_targets` — предполагаемые модули для приоритизации результатов поиска
- `investigation_depth` — глубина исследования (1 = только целевые файлы, 3 = рекурсивный обход зависимостей)
- `requires_tests` — нужно ли искать тестовые файлы

Эти поля покрывают 90% сценариев coding agent. Дополнительные поля (например, `affected_layers`, `security_sensitive`) добавляются по мере необходимости.

**Альтернативы:**
- ❌ Rule-based классификация — требует ручного обновления правил для каждого нового типа задач
- ❌ Keyword extraction без LLM — не понимает контекст ("почини" vs "добавь" vs "удали")
- ❌ Semantic search по описаниям задач — требует предварительно размеченного датасета

**Компромиссы:**
- Дополнительный LLM call (~500 токенов) на каждый запрос — оправдано экономией 3000-5000 токенов на исследовании
- Точность классификации ~80% (не 100%) — но даже неточная классификация сужает область поиска
...
**Почему pipeline именно в таком порядке: tree → search → read → graph → select?**

1. **tree (git ls-files)** — сначала получаем полную структуру проекта. Это дёшево (~100 токенов) и даёт понимание масштаба. Без структуры невозможно приоритизировать поиск.

2. **search (git grep)** — затем ищем по ключевым словам из TaskProfile. Поиск по всей структуре эффективнее, чем чтение всех файлов. git grep работает за миллисекунды даже на больших репозиториях.

3. **read (fs/read_text_file)** — читаем только найденные файлы, а не все. Это экономит 80-90% токенов по сравнению с чтением всего проекта.

4. **graph (build_dependency_graph)** — строим граф зависимостей только для прочитанных файлов. Это даёт понимание связей без анализа всей кодовой базы.

5. **select (выбор целевых файлов)** — финальный этап: выбираем файлы с учётом графа зависимостей. Файлы, от которых зависят другие, приоритетнее.

**Почему через ACP tools (terminal, fs), а не напрямую?**
- Единый интерфейс для LOCAL и REMOTE режимов
- Все операции выполняются на стороне клиента (где файловая система)
- Сервер не имеет прямого доступа к файлам в REMOTE режиме
- ACP tools уже поддерживаются существующей инфраструктурой

**Альтернативы:**
- ❌ Прямой доступ к файловой системе — не работает в REMOTE режиме
- ❌ RAG (vector embeddings) — требует предварительной индексации, избыточно для MVP
- ❌ Чтение всех файлов — слишком дорого по токенам для больших проектов

**Компромиссы:**
- Поиск через git grep/rg не понимает семантику кода — но для MVP достаточно точного совпадения
- Чтение файлов ограничено 20 файлами — может быть недостаточно для очень сложных задач
...
**Почему regex-based парсинг импортов в MVP, а не tree-sitter?**
Tree-sitter даёт точный AST-парсинг, но:
- Требует установки бинарных зависимостей для каждого языка
- Усложняет деплой (нативные библиотеки для Python)
- Избыточен для MVP — regex покрывает 95% случаев для Python/TypeScript/JavaScript

Regex-based подход:
- Работает без внешних зависимостей
- Легко расширяется (добавить новый язык = добавить новый regex)
- Достаточно точен для стандартных паттернов импортов

**Почему DFS для обхода зависимостей?**
- Простота реализации (~20 строк кода)
- Находит все транзитивные зависимости
- Предсказуемый порядок обхода (удобно для отладки)

**Альтернативы:**
- ❌ Tree-sitter — избыточен для MVP, усложняет деплой
- ❌ LSP (Language Server Protocol) — требует запуска language server для каждого языка
- ❌ Статический анализ через compiler — медленно, зависит от компиляции

**Компромиссы:**
- Regex не обрабатывает динамические импорты (`importlib`, `require()` с переменными) — но это <5% случаев
- Не различает типы импортов (type-only vs runtime) — достаточно для построения графа
...
**Почему ratios 20/50/20/10?**
- `system_context: 20%` — инструкции, AGENTS.md, структура проекта. Достаточно для понимания правил, но не доминирует над историей.
- `conversation_history: 50%` — основная рабочая область. Половина бюджета — это история диалога, потому что агент работает итеративно.
- `tool_outputs: 20%` — результаты чтения файлов, поиска, выполнения команд. Достаточно для контекста, но не бесконечно.
- `response_buffer: 10%` — резерв для ответа LLM. Если ответ не помещается — это проблема модели, не бюджета.

Эти ratios подобраны эмпирически на основе анализа типичных coding-задач. Они могут быть настроены через `codelab.toml`.

**Почему truncate середина (начало + конец), а не только конец?**
- Начало файла содержит импорты, определения классов, сигнатуры функций — критично для понимания
- Конец файла содержит реализацию, которую чаще всего нужно изменить
- Середина — часто boilerplate, который можно восстановить из контекста

**Альтернативы:**
- ❌ Равное распределение (25/25/25/25) — недостаточно места для истории
- ❌ Динамическое распределение — сложно предсказать, сколько нужно каждому компоненту
- ❌ Truncate только конец — теряем импорты и определения

**Компромиссы:**
- Фиксированные ratios не адаптируются к типу задачи — но настраиваются через конфиг
- Truncate середины может потерять важную логику — но это лучше, чем потерять всё
...
**Обоснование внутренних компонентов:**

#### TokenSlicer

**Почему отдельная дешёвая модель, а не основная?**
Суммаризация ответов субагентов происходит на каждом шаге маршрутизации. Если использовать основную модель (например, GPT-4 или Claude 3.5), стоимость будет чрезмерной:
- Основная модель: ~$0.03 за 1K токенов
- Дешёвая модель (GPT-4o-mini): ~$0.0003 за 1K токенов
- Разница в 100 раз при одинаковом качестве суммаризации

**Почему skip threshold?**
Маленькие ответы (<500 токенов) не нужно суммаризировать — они и так помещаются в контекст. Skip threshold экономит LLM calls для тривиальных случаев.

**Альтернативы:**
- ❌ Использовать основную модель — слишком дорого
- ❌ Простое усечение (truncate) — теряет ключевую информацию
- ❌ Не суммаризировать — переполнение контекста после 3-4 субагентов

#### ContextCompactor

**Почему двухфазный: Prune → Summarize?**
1. **Prune (удаление старых tool outputs)** — дешёвая операция без LLM. Удаляет результаты чтения файлов, поиска, выполнения команд. Экономит 30-50% токенов.
2. **Summarize (LLM-суммаризация)** — если prune недостаточно, сжимаем середину истории через LLM. Сохраняем первые 2 сообщения (system + initial prompt) и последние 3 (recent context).

**Почему сохраняем первые 2 + последние 3?**
- Первые 2: system prompt и начальная задача — без них LLM не понимает контекст
- Последние 3: недавние действия — без них LLM теряет нить разговора
- Середина: может быть сжата без критической потери информации

**Альтернативы:**
- ❌ Только prune — недостаточно для длинных сессий
- ❌ Только summarize — слишком дорого (каждый раз LLM call)
- ❌ Удалять всё кроме последних 5 — теряем системные инструкции

#### ChildSessionManager

**Почему child sessions, а не просто summary?**
- Summary теряет детали — если пользователю нужно углубиться в работу субагента, он не может
- Child session сохраняет полный контекст субагента (все tool calls, промежуточные результаты)
- Навигация в TUI: пользователь может переключаться между parent и child sessions
- Audit trail: полная история работы всех агентов

**Почему наследование mode от parent?**
- Безопасность: sub-agent не может переключиться в bypass mode, если parent в plan mode
- Предсказуемость: поведение sub-agent соответствует ожиданиям пользователя
- Простота: не нужно управлять mode для каждой child session отдельно

**Альтернативы:**
- ❌ Только summary — теряем детали, нет навигации
- ❌ Независимые сессии без связи — нет audit trail, сложно отлаживать
- ❌ Общий контекст для всех агентов — переполнение, нет изоляции
