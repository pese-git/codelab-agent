...
- [ ] Одна загрузка на запрос: команда передаёт загруженный объект в функцию
- [ ] Явное сохранение в конце транзакции `session/load`
- [ ] Тест: после `session/load` состояние **на диске** соответствует решениям обработчика
      (`cwd`, отмена вызовов, ответы на хвост) — проверять диск, а не мутированный объект
- [ ] Перевод транзакции на `SessionRepository` (фаза D ADR-006)
