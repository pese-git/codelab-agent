...
### 62. `session/load` решает судьбу turn'а по «последнему» ожиданию — полем, которым домен решать запрещает — ⬜ ОТКРЫТО (обнаружено 2026-08-07, разбор логов)

**Файлы:** `server/protocol/commands/session_load.py:108-116`.
...
**Направление:** спрашивать `outstanding_permissions` и решать по каждому ожиданию отдельно —
снимать осиротевшие, сохранять живые; `clear_active_turn()` оставить для случая, когда осиротели
все.

**Критерий приёмки:** загрузка сессии с двумя ожиданиями, одно из которых в реестре, сохраняет живое
и снимает сироту; гейт проверяется возвратом дефекта (решение по «последнему» ожиданию).

**Связано:** P1-61 (та же асимметрия на пути ответа, закрыта), P2-46, ADR-008 шаг 5.
