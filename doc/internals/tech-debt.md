...
### 7. Обновить зависимости (минорные/патч) — 🟡 ЧАСТИЧНО (2026-07-10)
...
> Все изменения проверены: `ruff` чисто, 7285 тестов passed. Установленные версии уже
> соответствовали новым границам, поведение не меняется.

**Осталось (нижние границы vs доступные версии):**

| Пакет | Нижняя граница | Доступная | Тип |
|-------|----------------|-----------|-----|
| `pydantic` | 2.11.0 | 2.13.4 | minor |
| `pydantic-settings` | 2.0.0 | 2.11.0 | minor |
| `aiohttp` | 3.12.15 | 3.13.5 | patch |
| `python-dotenv` | 1.0.0 | 1.2.1 | patch |

**Задачи:**
- [ ] Поднять оставшиеся нижние границы в `pyproject.toml`
