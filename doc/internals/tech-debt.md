...
### 63. Отмена turn'а отвечает за вызов, который в этот момент исполняется: два `role: tool` на один id — ⬜ ОТКРЫТО (обнаружено 2026-08-10, разбор логов; **приоритет P1**)
...
**Направление:** назвать владельца. `cancel_active_tools` пропускает вызовы, находящиеся в полёте
(их ответ придёт от исполнителя), либо ответ помечается идемпотентным по `answer_id` —
`add_tool_result` отклоняет второй ответ на тот же id. Второе сильнее: закрывает и будущие двери,
и делает инвариант «один ответ на вызов» невыразимо нарушаемым, как это сделано с фазами turn'а.

**Критерий приёмки:** отмена turn'а с вызовом в полёте даёт ровно один `role: tool` на его id;
гейт проверяется возвратом дефекта. Живая проверка — отмена во время `terminal/wait_for_exit`
(воспроизводится надёжно: окно ожидания 0.5–1.5 с).
