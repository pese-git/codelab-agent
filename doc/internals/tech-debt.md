...
**Замер, которого не хватает и который идёт первым шагом:** пути в логах не печатаются вовсе (ноль
вхождений `path=` за прогон 2026-08-10), поэтому «сколько чтений уходит за пределы `cwd`»
**неизвестно** — а именно от этого зависит цена включения границы каталога.
