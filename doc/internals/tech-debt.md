...
Осталось ровно два пункта, и оба — не «дописать дверь»:

1. **Мёртвая вторая дверь.** `TurnLifecycleManager.set_turn_phase` по-прежнему существует и
   по-прежнему **не имеет вызывающих в проде** — только в тестах. Замер того же класса показал, что
   мёртв не он один: из восьми методов класса живы **два** (`create_active_turn`, `finalize_turn`),
   а `mark_cancel_requested`, `is_cancel_requested`, `set_turn_phase`, `get_turn_phase`,
   `resolve_stop_reason`, `should_handle_cancel` не зовёт никто. `resolve_stop_reason` вдобавок
   дублирует `normalization.normalize_stop_reason`, который живой путь и использует. Удаление — не
   решение автора пункта: оно требует явного выбора пользователя.
