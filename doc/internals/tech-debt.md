...
2. **Латентная асимметрия, отложенная шагом 5.2 и не закрытая шагом 5.3.** Turn **без**
   `prompt_request_id` не снимается на двух путях (`prompt/turn_state.py::finalize_active_turn`,
   `client_rpc_response.py::_finalize_turn`): оба выходят раньше снятия. Тогда на диске остаётся
   `active_turn` с фазой паузы — ровно симптом этого пункта. С Zed случай не наступает (промпт всегда
   приходит запросом с id, что видно по `answered=True` в `turn_finished`), поэтому он латентный, а
   не наблюдаемый; но снятие там было бы изменением поведения и требует своего живого прогона.
