...
> Анализ логов после транзакции 6 (2026-07-31): добавлен **P2-50** — отменённый пользователем `terminal/wait_for_exit` попадает на диск как `failed`, а модели уходит результат с признаком ошибки (уровень лога уже понижен ранее, осталась семантика статуса и текста).
...
**Связано:** P2-41 (`build_identity` — им и различили процессы), P2-43 (точки входа).
...
### 50. Отменённый пользователем RPC инструмента выглядит как сбой: вызов `failed`, модель слышит «ошибка» — ⬜ ОТКРЫТО (обнаружено 2026-07-31, анализ логов)
