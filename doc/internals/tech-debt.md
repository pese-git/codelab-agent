...
**Воспроизведено под сторонним клиентом — направление поиска было неверным (2026-08-04).** Прежнее
направление отправляло искать двойной `Popen` в нашем клиенте (`client/infrastructure`). Но пары
воспроизводятся при запуске сервера из Zed, где нашего клиента в схеме нет вовсе:
`74577`/`74578` (05:10:29.423895 / .423893) и `76979`/`76980` (05:12:26.097812 / .097810), оба
`transport=stdio`, `version=0.2.0` из pipx-сборки. Расхождение метки **2 микросекунды** исключает
два последовательных запуска IDE: столько не занимает даже подготовка `Popen`. Значит удвоение
происходит **на нашей стороне**, уже после того, как процесс дошёл до логирования `build_identity`,
и искать его надо в точке входа, а не у вызывающего. Холостые близнецы снова по 4343 байта и снова
заканчиваются на `stdio transport started`.
