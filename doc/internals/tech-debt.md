...
**Пользователь узнавал об этом только из лога (обнаружено 2026-08-17 сверкой документации).**
Честный контракт был сделан на уровне рантайма — предупреждение в лог, — но
`doc/product/user-guide/llm/fallback-resilience.md` описывал fallback как работающую функцию:
конфигурация, стратегии, схема переключения, примеры `export CODELAB_FALLBACK_*`. То есть
единственным способом узнать правду был запуск сервера и чтение `warning`. Страница исправлена
(проход 2 сверки документации): предупреждение вынесено в начало, env-раздел удалён как
недействующий (см. P1-33). Это снимает симптом, но не пункт.
...
предупреждающим; документация описывает состояние функции без запуска сервера.
...
**Симптом измерен исполнением (2026-08-17): вложенные секции из окружения не заполняются.**
Сверка `doc/product` с кодом нашла **11 задокументированных переменных, которые молча не читаются**:
`CODELAB_FALLBACK_ENABLED/-STRATEGY/-ORDER`, `CODELAB_AGENTS_STRATEGY/-DEFAULT_MODEL/-MAX_STEPS/
-FALLBACK_STRATEGY`, `CODELAB_OBSERVABILITY_ENABLED/-EXPORT_DIR/-FLUSH_INTERVAL/-MAX_FILE_SIZE`,
плюс `CODELAB_LLM_TIMEOUT_WRITE/POOL`. Проверено не чтением конфига, а запуском на реальном пути
`AppConfig.load()`: при `CODELAB_OBSERVABILITY_ENABLED=false` значение остаётся `True`, при
`CODELAB_AGENTS_STRATEGY=hierarchical` — `single`. Причина в том же расхождении: `model_config`
задаёт только `env_prefix="CODELAB_"`, а ручная лестница разбирает по одному полю
(`CODELAB_LLM_TIMEOUT_READ/CONNECT` разобраны и **работают**, `WRITE/POOL` — нет).

Контраст, подтверждающий диагноз: `CODELAB_CONTEXT_*` работают — у Context Manager **свой**
загрузчик (`agent/context/config_loader.py`, префикс + имя поля), то есть третья система разбора
окружения рядом с двумя описанными выше.

Это не отменяет нюанс про два направления env: добавление `env_nested_delimiter` сняло бы
молчаливое игнорирование, но не заменило бы `${VAR}`-раскрытие внутри TOML. В задачах ниже
направление уже учтено; здесь фиксируется цена бездействия — документация обещала настройку,
которой нет, и это обнаружилось только сверкой.

**Задачи:**
- [ ] Спроектировать источники `settings_customise_sources` (init/env/toml-with-expansion) с сохранением текущего приоритета
- [ ] Свести к одному разбору окружения и третью систему — `agent/context/config_loader.py`
...
### 34. `ContextGatherer` content-search: O(термы × файлы) чтений через ACP RPC — ✅ ЗАКРЫТО (2026-08-17: поиск сужен до `project/search_content`; ждёт живого подтверждения)

**Закрыто не тем решением, что предлагалось, и по уточнённой причине (2026-08-17, P2-57).** Две
поправки к разбору ниже. Первая: «`_read_file` **не обращается к `FileContentCache`**» неверно —
путь сборки идёт через `FileCacheDecorator`, и замер двух прогонов показал, что реальный RPC равен
числу **уникальных** путей (39 из 176 исполнений, 31 из 155); до fs-хендлера чтения доходили не
каждый раз, 160 completion-записей считали исполнения, а не RPC. То есть O(термы × файлы) стоило не
сетевых round-trip'ов, а обходов гейта разрешений и записей в лог.

Вторая: предложенное решение — «прочитать каждого кандидата один раз и матчить термы в памяти» —
оставило бы поиск реализованным через чтение, то есть сохранило бы и предъявление гейту «чтения
файла» вместо «поиска», и окно `[:30]` у потребителя. Вместо него поиск сужен до узкой возможности
`project/search_content` (ADR-009 §6): сборщик получает пути, файлы ради поиска не читаются вовсе,
и число чтений становится равно числу собранных файлов, а не числу проверенных. Критерий приёмки
выполнен строже заявленного (не `M`, а `|собранные|`); равенство результата закреплено тестами
`test_search_in_files_by_content` и `test_gather_reads_only_what_it_collects`.

Ниже — исходный разбор 2026-07-23, сохранён как измеренная история.
...
**Последствие измерено на пользовательской документации (2026-08-17).** Расхождение перестало
быть внутренним: сверка `doc/product` с кодом нашла **14 задокументированных флагов
`codelab serve`, которых у достижимой точки входа нет** — `--llm-provider`, `--llm-model`,
`--llm-api-key`, `--llm-timeout-*`, `--storage`, `--config`, `--log-json`, `--log-file`,
`--auth-api-key`, `--fallback-enabled/-strategy/-order`, `--verbose`. Справочник CLI, руководство
по установке, страницы IDE-интеграций и README описывали их как рабочие: пользователь, следующий
инструкции, получает ошибку разбора аргументов. Документация выправлена по фактическому списку из
семи флагов (коммит прохода 2), но это лечение симптома — расхождение точек входа осталось.

Это меняет вес пункта: до замера он выглядел внутренней неопрятностью («недостижимый дубль»),
после — дефектом, видимым снаружи. Приоритет пересмотреть при взятии.

**Критерий приёмки:** одна точка входа для `codelab serve`; возможности из короткого списка
доступны (`--storage` обязательно); документация не описывает несуществующий кеш; список флагов в
`doc/product/reference/cli.md` совпадает с выводом `codelab serve --help`.
...
**Воспроизведено снова 2026-08-12 (`sess_2cb45270e19a`, `pid 71370`) — уже на другой модели**
(`litellm/openai/MiniMax-M3`, тогда как прежние наблюдения шли на локальном `lmstudio/qwen3.6-35b`).
Один `no_json_found → heuristic` за прогон. Существенно, что смена провайдера дефект не убрала:
причина не в конкретной модели, а в том, что разбор чужого ответа держится на жадной регулярке и не
имеет признака «классификация не состоялась». Пробел наблюдаемости с 2026-08-06 сохраняется —
причину отказа по логу по-прежнему не отличить.
...
### 54. `active_turn.phase` никто не возвращает в `running`, а валидирующий сеттер фазы мёртв — ✅ ЗАКРЫТО (обнаружено 2026-07-31, анализ логов; закрыто 2026-08-12: единственная дверь — шагом 2 ADR-008, персистентность снятия — 2026-08-11, асимметрия без `prompt_request_id` и мёртвая вторая дверь — 2026-08-12)
...
**Второй критерий приёмки закрыт (2026-08-11).** «После штатного завершения `active_turn: null`» не
выполнялся до сегодняшнего дня, и это подтвердилось живым прогоном уже **после** ввода шва завершения
(`sess_1fb7b8156367`, ревизия 336): в логе `turn_finished cause=completed`, в документе — `active_turn`
с `phase: running`. Причина не в фазе: снятие turn'а в штатном пути **не персистировалось** —
`BackgroundExecutor.complete_active_turn` грузил сессию read-only и не сохранял её. Симптом гасил
следующий промпт, снимавший turn как устаревший, поэтому враньё жило до него — включая перезапуск
процесса, где оно превращалось в «осиротевшее разрешение».

Исправлено командой над свежей копией (`SessionCommands.apply`), а не прямым сохранением: решение,
принятое над копией из `load_session`, на диск бы не уехало — ровно дефект P2-42. Холостой вызов
транзакции не открывает: метод зовут все три транспорта, а транзакция сохраняет безусловно.
Гейт — на **файловом** бэкенде (`test_turn_completion_persists.py`): на `InMemoryStorage`
«мутировали и не сохранили» невыразимо. Проверен возвратом дефекта.

**Остаток пересчитан по коду (2026-08-12) — прежняя формулировка устарела.** «Запись фазы идёт из
нескольких мест» больше не соответствует коду: единственный `self.phase = ...` в проекте стоит внутри
`TurnState.transition_to` (`domain/session.py`), матрица `ALLOWED_TURN_PHASE_TRANSITIONS` применяется
на каждом переходе, а отказ пишет `turn_phase_transition_rejected` — событие, прежде недостижимое.
Пять прямых присваиваний из `tool_processor` и `directives` исчезли в шаге 2 ADR-008. Возврат в
`Running` есть: `close_permission` отдаёт `Running`, когда закрыто последнее ожидание. То есть
**единственная дверь закрыта**, и оба критерия приёмки, кроме одного латентного случая ниже,
выполнены.

Осталось ровно два пункта, и оба — не «дописать дверь». **Оба закрыты 2026-08-12.**

1. ~~**Мёртвая вторая дверь.**~~ ✅ **Закрыто 2026-08-12 решением пользователя «удаляй».** Из восьми
   методов `TurnLifecycleManager` вызывающих в проде имели **два** (`create_active_turn`,
   `finalize_turn`); `mark_cancel_requested`, `is_cancel_requested`, `set_turn_phase`,
   `get_turn_phase`, `resolve_stop_reason`, `should_handle_cancel` не звал никто, только тесты. Это
   след трёх переездов — фаза уехала в домен (ADR-008 шаг 2), снятие turn'а получило владельца (шаг
   5.2), нормализация stop reason жила в `prompt.normalization`, — и каждый оставлял здесь копию.

   **Удалено переносом, а не вычёркиванием.** Поведение сохранено там, где живёт: матрица переходов
   перенесена в `tests/server/domain/test_turn_phase_transitions.py` на `TurnState.transition_to`
   (иначе исчезло бы **единственное** её покрытие — доменные тесты проверяли только чтение фазы из
   wire); предупреждение о подмене stop reason перенесено в `prompt.normalization.normalize_stop_reason`
   и получило свой гейт. Заодно стало проверяемым то, что прежде было недостижимо: отказ перехода
   пишет `turn_phase_transition_rejected` — пока единственным входом был мёртвый метод, событие не
   могло сработать ни разу.

   **Гейт на невозвращение:** `TestLiveSurfaceOnly::test_public_surface_is_exactly_two_methods`
   требует, чтобы публичная поверхность класса равнялась `{create_active_turn, finalize_turn}`.
   Проверен возвратом дефекта — возвращённый `get_turn_phase` валит ровно его. Без гейта копии
   накопились бы снова тем же способом, каким накопились эти шесть.
2. ~~**Латентная асимметрия, отложенная шагом 5.2 и не закрытая шагом 5.3.**~~ ✅ **Закрыто
   2026-08-12.** Turn **без** `prompt_request_id` не снимался на двух путях
   (`prompt/turn_state.py::finalize_active_turn`, `client_rpc_response.py::_finalize_turn`): оба
   выходили раньше снятия, и на диске оставался `active_turn` с фазой паузы — ровно симптом этого
   пункта. Guard'ы сняты: `finish_turn` уже умел этот случай — снимает turn и возвращает `None`, не
   строя ответа, поэтому вся «смена поведения» свелась к тому, что состояние на диске перестало врать.
   **Гейтов на прежнее поведение не было вовсе** — снятие guard'ов не сломало ни одного из 786 тестов
   пути; оба добавлены и проверены возвратом дефекта, на файловом бэкенде. Случай латентный: с Zed
   промпт всегда приходит запросом с id (`answered=True` в `turn_finished`), поэтому живого прогона он
   не требует — наблюдать нечего.
...
### 56. Гейт разрешений живёт у вызывающего, а не у реестра: Context Manager запускает команды мимо политики — 🟡 ЧАСТИЧНО (обнаружено 2026-08-04; измерено шагом 0 ADR-009 2026-08-10: 94.5% защищённых инвокаций без гейта; **приоритет P1**)
...
**Блокировка снята решением 2026-08-10: `ADR-009`.** Долг был отложен «вопросом политики разрешений
целиком», и это оказалось верной постановкой, но не тупиком: политика перестаёт быть одна на всех,
когда становится **функцией субъекта**. Отсюда снялось и главное возражение против гейта у реестра —
«Context Manager начнёт спрашивать разрешение»: для субъекта `context` исход `Ask` невыразим по
конструкции, а чтения внутри рабочего каталога разрешает политика.

Разведка по коду добавила к записи три факта, меняющие оценку цены:

1. **Шов уже единственный.** Оба пути сходятся в `ToolRegistry.execute_tool` (turn —
   `tool_processor.py:1081`, Context Manager — `gatherer.py:409,524,550,610` и `manager.py:202`);
   переносить нужно место проверки, а не архитектуру. MCP — второй вход
   (`MCPToolExecutor.execute_tool`), его придётся накрыть тем же приёмом.
2. **У флага `requires_permission` есть второй потребитель** — `registry.py:149` фильтрует по нему
   набор инструментов для модели. Смена смысла флага задевает выдачу инструментов, а не только гейт.
3. **Аннотации MCP уже разобраны** (`models.py:213-216`) и уже признаны надёжными для вывода `kind`
   (`tool_adapter.py:157-159`), тогда как `requires_permission` для MCP проставлен `True` подряд
   (`tool_adapter.py:209`). Вторая ось игнорирует данные, которым первая доверяет.

**Шаг 0 ADR-009 сделан и снят живьём (`sess_981b683455b0`, 2026-08-10) — долг измерен точно.**
518 инвокаций, ни одной `unknown`:

| | `context` | `model` |
|---|---|---|
| инвокаций | 487 (94%) | 31 (6%) |
| защищённых, но без гейта | **485** | 0 |

**94.5% защищённых инвокаций (485 из 513) исполняются мимо решения пользователя.**

**Формулировку долга замер уточнил, и её стоит поменять: тяжесть не в чтениях, а в запуске команд.**
`inside_cwd=True` в 509 случаях из 509 — граница рабочего каталога действует для обоих путей
(`filesystem.py:180-187`), поэтому 485 незагейченных чтений ограничены каталогом и по риску близки к
нулю. Опасен один случай:

```
subject=context  terminal/create  command='find . -type f'   gated=False
subject=model    terminal/create  command='ls -la'           gated=True
```

Один инструмент, одна декларация `requires_permission=True`, разная судьба — по вызывающему. У
терминала границы пути нет и быть не может: команда исполняется целиком. Пользователь, включивший
запрос разрешения на исполнение команд, для команды Context Manager его не получает.

**Решение по опасной половине принято 2026-08-10 и оно не про политику (ADR-009 раздел 6).** Разбор
показал несоответствие полномочий: Context Manager нуждается в «перечислить файлы», а держит
«исполнить произвольную команду». Вся его работа с терминалом — одна функция `_bootstrap_project_files`
(`gatherer.py:512`) с литеральной командой `find . -type f`. Поэтому вопрос «разрешить ли ему
запускать команду?» неотвечаем, а «разрешить ли перечислять файлы?» — отвечаем.

Полномочие сужается до перечисления, право на произвольную команду исчезает **по построению**.
Белый список команд отклонён: валидация shell-строк проигрывает метасимволам и конвейерам, а множество
команд не замкнуто уже сегодня. После сужения политика сводится к одному правилу — перечисление есть
чтение, риск тот же, что у 485 уже разрешённых чтений внутри `cwd`.

**Связано:** **ADR-009** (решение), ADR-006 (шаг 3 — там же зафиксированы два вызывающих цепочки
`tools/`), ADR-008 раздел 7 (разрешение как свойство инвокации), P2-57 (тот же вызывающий, вопрос
окупаемости его чтений), P2-58 (терминалы, создаваемые этим путём).

---

### 57. Context Manager не окупается: 161–239 чтений на 7–20 файлов, метрика не отражает реальные чтения — ✅ ЗАКРЫТО (обнаружено 2026-08-04; 2026-08-17: разложение сделано, поиск сужен до возможности, окупаемость измерена аблацией — зависит от широты задачи; следствие вынесено отдельным пунктом)

**Файлы:** `tools/definitions/project.py` (`project/search_content` — владелец команды),
`agent/context/gatherer.py` (сбор и чтения), `agent/context/manager.py`
(`_refresh_dirty_sources`, `_read_file`), `agent/context/task_analyzer.py`, метрика
`files_read` в `record_context_build`.

**Разложение чтений сделано (2026-08-17, прогоны `codelab-70854`, `codelab-74850`) — и оно
изменило диагноз.** Разложение по источникам, значившееся «первой работой при взятии», выполнено
замером по коду и логу. Дискриминатором послужило поле `has_metadata` в `tool handler execution
completed`: попадание в кэш возвращает `metadata={"from_cache": True}`, реальный RPC метаданных не
несёт.

| прогон | исполнений `fs/read_text_file` | из них кэш | реальный RPC | уникальных путей |
|---|---|---|---|---|
| `codelab-70854` | 176 | 137 | 39 | 39 |
| `codelab-74850` | 155 | 124 | 31 | 31 |

Реальный RPC **ровно равен** числу уникальных путей в обоих прогонах: каждый файл уходит к клиенту
один раз, все повторы обслуживает `FileContentCache`. Тайминги сходятся — первый термин стоил 65 мс
на 30 чтений, последующие 4–5 мс на те же 30.

Отсюда две поправки. **Направление 2 снято как опровергнутое:** кэш задействован, чтения не
повторяются — путь сборки идёт через `FileCacheDecorator`, потому что `read_handler` проставляет
`operation="read"`, а декоратор ловит именно этот ключ. **Цена лежала не в диске:** гейту разрешений
предъявлялось 176 «чтений файла» вместо 5 «поисков» (P1-56), метрика считала не ту величину, и
каждое из 137 попаданий всё равно проходило probe, гейт и лог — ~350 строк лога на сборку ради 5–6
доставленных файлов.

**Побочная находка того же замера, дороже исходного вопроса.** `_search_in_files` брала
`files_to_check[:30]` — первые 30 путей списка проекта, одни и те же для каждого термина. При 41
файле в проекте поиск по содержимому **структурно не видел хвост**: файл на 35-й позиции не мог быть
найден ни по одному термину. Это не настройка, а протёкший к потребителю параметр реализации поиска
— и потому у него не было владельца.

**Взято (2026-08-17): поиск сужен до узкой возможности `project/search_content`** — тем же приёмом,
что ADR-009 раздел 6 применил к перечислению. Сборщику нужна одна операция «дай пути файлов, где
встречается термин», а в наличии была только возможность прочитать **один** файл, и он собирал
первую из второй сам. Теперь команду формирует единственный владелец (`tools/definitions/project.py`),
потребитель передаёт только термин, а носителем остаётся терминал как деталь реализации.
Существенные решения:

* **инъекции нет по построению, а не по экранированию:** `terminal/create` принимает `command` и
  `args` раздельно, поэтому термин уходит элементом argv и shell его не разбирает; `--` закрывает
  список опций;
* **семантика сохранена дословно:** `grep -r -l -I -i -F` — регистронезависимое совпадение
  подстроки, ровно то, что делало сравнение `term.lower() in content.lower()`;
* **отсутствие совпадений — успех с пустым выводом:** `wait_for_exit` считает успехом только
  `exit_code == 0`, а grep отдаёт 1 при «не найдено», поэтому без явной ветки потребитель не отличил
  бы «такого нет» от «поиск сломался»;
* **область поиска ушла к владельцу**, и слепой хвост исчез не потому, что его починили, а потому,
  что для него не осталось места.

**Первый живой прогон нашёл дефект в самой возможности (2026-08-17, `codelab-6958/11074/13599/15712`,
`~/flutter_app`).** 44 вызова `project/search_content` из 44 — `success=False`: поиск не дал ни
одного кандидата, и в контекст файлы попадали только совпадением по пути, fallback'ом и обходом
зависимостей. Причина оказалась не той, что предполагалась (гипотеза «клиент не исполняет `args`»
**опровергнута**: в логе видно и `args`, и найденный `counter_page.dart`): `grep` печатал
совпадения и **следом** возвращал 2 из-за одного нечитаемого файла в дереве —
`./.codegraph/daemon.sock: Operation not supported on socket`. У grep «произошла ошибка» включает
любой нечитаемый файл, поэтому на живом проекте код 2 — норма, а `_run` трактовал ненулевой код как
отказ и выбрасывал верный результат целиком.

Закрыто правилом «**исход определяется выводом, а не кодом возврата**»: коды, штатные для команды,
объявляет её владелец (у grep их два — 0 и 1), а непустой вывод при любом другом коде даёт успех,
помеченный `partial=True`, с событием `project.command.partial`. Локально дефект не воспроизводился
(macOS-grep возвращал 0 на том же дереве), а тесты его скрывали: мок обнулял вывод на ненулевом
коде — модель, которой живой grep не следует. Мок исправлен, случай закреплён
`TestUnreadableFilesInTree`.

**Наблюдаемость, без которой причину пришлось бы угадывать.** Тем же прогоном выяснилось, что отказ
возможности был безымянен: код возврата не доезжал до вызывающего, а `content_search.failed` жил на
уровне `debug` и в файле лога отсутствовал. Добавлены `project.command.failed` и
`project.command.partial` (уровень `warning`, с фактическими `command`/`args`/`exit_code`), отказ у
потребителя поднят до `warning`. Именно эти две строки и дали причину со второго прогона.

Побочно изменилось наблюдаемое поведение: пустое перечисление больше не обнуляет поиск по
содержимому — найденное существует независимо от списка у потребителя. `parse_find_output`
переименована в `parse_path_listing` (её обслуживают обе возможности) и отбрасывает диагностику
обеих утилит.

✅ **Подтверждено живьём** (`sess_f00c9a547403`, 2026-08-17, `pid 29485`, stdio через Zed,
`~/flutter_app`; копия pipx сверена с деревом):

| Ожидание | Результат |
|---|---|
| поиск идёт возможностью и работает | 5 × `project/search_content`, **5 успешных из 5** (было 0 из 44) |
| слепая зона снята | `counter_page.dart` найден **поиском** по трём терминам и попал в baseline (прежде приходил только обходом импортов) |
| код 2 не отменяет результат | 5 × `project.command.partial`, ноль `content_search.failed` |
| область поиска шире прежнего окна | `raw_count=31` на термине `widget` — за пределами прежних 30 файлов |
| чтений столько, сколько собрано | 25 чтений `subject=context` на 20 собранных файлов (было 176 на 6) |
| терминалы освобождаются | `live=0`, `unwaited=0`; ноль `error`/`critical` |

Цена сборки: 5 поисков ≈ 910 мс (~180 мс на термин) против прежних ~80 мс на неполный поиск по 30
файлам — плата за полноту, замеренная и принятая.

**Окупаемость измерена аблацией (2026-08-17, `~/flutter_app`, MCP выключен, четыре прогона).**
Предложенная метрика «пересечение путей `subject=context` и `subject=turn`» сначала дала ложный
идеал: во всех прогонах у модели было ноль чтений — но не потому, что контекст окупился, а потому
что у неё подключены MCP-серверы `codegraph`/`cocoindex-code` и разведка шла ими. Метрика,
считающая один канал, молча объявляет успехом уход в другой. Поэтому мерилось иначе: один и тот же
промпт с `enabled = true` и `enabled = false`, MCP выключен, каждая сессия новая.

| Промпт | Контекст | Чтений моделью | Итераций цикла | Время прогона |
|---|---|---|---|---|
| П1 «где описан FloatingActionButton» | вкл (`36729`) | **0** | 0 | 24.4 с |
| П1 | выкл (`44668`) | 1 + 2 команды в терминале | 3 | 20.3 с |
| П2 «связь CounterBloc → use case → репозиторий» | вкл (`41074`) | **0** | 0 | 37.9 с |
| П2 | выкл (`46771`) | **13** + 2 команды | 6 | 44.1 с |

**Ответ зависит от широты задачи, и это главный результат.** На широкой задаче контекст окупается:
13 чтений, 2 команды и 6 итераций цикла заменяются одной сборкой, прогон короче на 6.2 с. На узкой —
**не окупается**: экономия одно чтение, а прогон длиннее на 4.1 с, ровно на стоимость `task_analyze`
(3.4 с) и сборки (1.1 с).

Качество сбора на П2 измеримо тем же прогоном — сравнением собранного с тем, что модель
затребовала сама: **полнота 10/13** (не собраны `get_counter_usecase.dart`, `counter_event.dart`,
`counter_state.dart`), **точность 10/20** — половина собранного модели не понадобилась.

**Следствие, оставленное открытым:** сборка запускается независимо от того, узкая задача или
широкая, хотя на узкой она чистый убыток. `TaskAnalyzer` уже классифицирует задачу и уже стоит
3.4 с — то есть решение «собирать ли» принимается после того, как оплачена его самая дорогая часть.
Это отдельный пункт (кандидат на P2-68), и он про порядок стадий, а не про поиск.
...
2. ❌ Кешировать содержимое между turn'ами сессии — **опровергнуто замером 2026-08-17:**
   `FileContentCache` задействован, реальный RPC равен числу уникальных путей.
3. ✅ Разложить чтения по источникам и срезать самый дорогой — сделано; источником был
   content-search (P2-34), он сужен до `project/search_content`.

**Критерий приёмки:** число `fs/read_text_file` за turn сопоставимо с числом собранных файлов
(единицы, а не сотни); в метриках видно и то, и другое. Первая половина закрыта гейтом
`test_gather_reads_only_what_it_collects` (чтений ровно столько, сколько собранных файлов) и ждёт
живого подтверждения; вторая — за направлением 1.
...
### 58. Мёртвые алиасы терминалов после рестарта: гарантированно провальные вызовы и реестр, который только растёт — ✅ ЗАКРЫТО (2026-08-05: «мёртвые алиасы» закрыты живьём; 2026-08-07: двойная выдача alias'а закрыта эпохой процесса; 2026-08-12: владение освобождением закрыто ADR-008 шагом 5.3 и подтверждено живьём, `sess_6574c534181b`, `released=1`, `live` → 0)

**Владение освобождением закрыто (2026-08-12, ADR-008 шаг 5.3).** По спецификации освобождает
**агент** (`10-Terminal.md:109-111`), а фактический приобретатель ресурса от лица модели — turn.
Замер семи прогонов: модель освободила остаток дважды, пять раз не освободила, то есть гарантия была
вероятностной. Остаток освобождает `protocol/turn_terminals.py::TurnTerminalReleaser` на путях, где
turn кончается по воле модели **и** которые исполняются в фоновых задачах: штатное завершение и
ошибка пайплайна. Освобождается только дожданный остаток (`waited=True`) — окно живой команды
наблюдалось живьём трижды, в последний раз `wait_for_exit` завершился через 8 с после отмены.

Два хвоста осознанно оставлены открытыми, оба разобраны в ADR-008:

* **путь отмены не покрыт** — stdio отправляет в фоновую задачу только `session/prompt`, поэтому
  клиентский RPC на пути `session/cancel` взаимоблокируется (измерено живьём: отмена перестала
  отвечать клиенту). Нужна фоновая задача с контролем жизненного цикла либо отправка `session/cancel`
  в фон на уровне транспорта;
* **утечка при смерти процесса** — реестр нового процесса пуст по построению, освобождать терминалы
  прошлого некому. Покрыть шагом 5.3 нельзя: восстановление связки вернуло бы мёртвые дескрипторы,
  ради устранения которых её и перестали персистировать.
...
### 64. `project_structure` пишут два владельца с разными фильтрами — ✅ ЗАКРЫТО (обнаружено 2026-08-11 разбором логов, закрыто в тот же день)

**Файлы:** `tools/executors/decorators/project_structure.py`, `agent/context/gatherer.py`,
`handlers/prompt_orchestrator.py` (регистрация).

**Симптом — измерен, не выведен** (`sess_b2a293adc9fd`, 2026-08-11):

```
project_structure.auto_saved       filtered_files=48   ← ProjectStructureDecorator
context.gather.bootstrap.complete  filtered_files=41   ← ContextGatherer, 2 мс спустя
```

Одна и та же структура проекта записывается дважды подряд, разными фильтрами, и побеждает вторая
запись. Расхождение 48 против 41 — не ошибка одного из них: у декоратора свой `IGNORE_DIRS`, у
сборщика — свой `filter_paths`.

**Причина.** Декоратор написан, чтобы извлекать структуру из команд **модели** (`find`, `ls`), и
перехватывает `terminal/create` + `terminal/wait_for_exit`. Bootstrap сборщика контекста ходил теми
же тремя вызовами, поэтому попадал под перехват заодно — и получал второго писателя, о котором не
знал. **Не регрессия шага 2 ADR-009:** до сужения полномочия было то же самое, шаг лишь сделал
расхождение видимым в одной строке лога.

**Решение: у каждого пути один писатель.** Возможность `project/list_files` регистрируется на
**недекорированном** executor'е — структуру на этом пути пишет `ContextGatherer`, который её и
разбирает. Команды модели по-прежнему идут через декоратор: там владельца нет и быть не может, модель
о `project_structure` не знает. Экземпляр executor'а общий: реестр alias'ов терминалов один на
процесс.

**Итоговое состояние не изменилось** — на диск и раньше уезжали 41 файл сборщика; исчезли лишняя
запись и ревизия между ними.

✅ **Подтверждено живьём** (`sess_d2646f8ff117`, 2026-08-11, stdio через Zed): `bootstrap.complete
filtered_files=41` без сопровождающего `project_structure.auto_saved` — прежней пары «48, следом 41»
нет. Записи декоратора остались только у команд модели, ради которых он и написан.

**Связано:** ADR-009 раздел 6 (сужение полномочия, которым находка вскрылась), P2-58 (владение
терминалом на том же пути).

---

### 65. `project_structure` перезаписывается частичным листингом модели — ✅ ЗАКРЫТО (обнаружено и закрыто 2026-08-11, разбор логов)

**Файлы:** `tools/executors/decorators/project_structure.py` (`_handle_terminal_wait`),
`agent/context/gatherer.py` (`_list_project_files` — потребитель).

**Симптом — измерен, не выведен** (`sess_d2646f8ff117`, 2026-08-11, один turn):

| время | писатель | что записано |
|---|---|---|
| 12:05:57 | `ContextGatherer` (bootstrap) | **41** файл — полная структура проекта |
| 12:07:00 | декоратор, команда модели `find … -name "*.dart" …` | **34** |
| 12:07:00 | декоратор, команда модели `ls -la` | **8** |

В документе осталось **8**. Дальше Context Manager отбирает кандидатов по этим восьми путям вместо
сорока одного — то есть качество сборки контекста определяется тем, какую команду модель случайно
выполнила последней.

**Причина.** У `project_structure` два писателя с разными правами на истину: сборщик пишет **полный**
листинг проекта, декоратор — результат **произвольной** команды модели, и оба пишут безусловно.
Декоратор не различает «полный листинг» и «листинг одного каталога»: `ls -la` для него такой же
источник структуры, как `find . -type f`.

**Не регрессия P2-64 и не её остаток.** Там конфликт был одновременным (две записи подряд на одном
пути bootstrap'а) и снят разделением путей; здесь — разнесённый во времени, и снимается решением о
том, кто вправе перезаписывать структуру.

**Направления (решить при взятии, замер обязателен):** писать только при отсутствии структуры;
либо различать полноту листинга по команде (`find . -type f` против `ls`); либо снять запись у
декоратора вовсе — его первоначальная задача (дать структуру, когда bootstrap'а ещё не было) после
ввода `project/list_files` могла исчезнуть, и это надо проверить замером, а не рассуждением.

**Воспроизведено вторично (`sess_f50f426f9619`, 2026-08-11, тот же день, другой прогон):** bootstrap
положил 41, `find lib -type f` дал 26, `find test -type f` — **5**; в документе осталось 5. Форма та
же, набор команд другой, то есть дефект не зависит от того, какие именно листинги запросит модель.

**Критерий приёмки:** после turn'а, в котором модель выполняла листинги подкаталогов,
`project_structure` в документе не меньше того, что положил bootstrap.

**Замер решил выбор между тремя вариантами, и ответом оказалось удаление (2026-08-11).**
Проверялось по коду, кто вообще потребляет структуру:

1. Потребитель **один** — `ContextGatherer` (`gatherer.py:484` читает, `:561` пишет). Больше её не
   читает никто: в `config_specs` её нет, наружу она не отдаётся, в продуктовой документации не
   упомянута.
2. Потребитель **сам** её заполняет: пустая структура → `_bootstrap_project_files`
   (`gatherer.py:172-175`). То есть декоратор не мог дать ему ничего, чего тот не получит сам.
3. Значит эффект записи декоратора строго отрицательный: она не добавляет возможности, а **заменяет**
   полное перечисление частичным — и именно этот остаток сборщик читает после рестарта процесса.

Поэтому выбран третий вариант — снять запись у декоратора. Поскольку кроме неё декоратор ничего не
делал, удалён он целиком вместе с тестами (≈250 + ≈440 строк). Это удаление функциональности, и оно
названо прямо: «структура проекта из команд модели» больше не извлекается. Заменять нечем и не нужно —
её единственный потребитель перечисляет файлы сам, узкой возможностью `project/list_files`.

**Гейт:** `test_gatherer_is_the_only_writer` — писатель `project_structure` в исходниках ровно один;
проверен возвратом дефекта (добавленная вторая запись валит его). Плюс тест «нет структуры — есть
перечисление»: потребитель не остался без данных.

**Цена, названная честно:** структура теперь обновляется только bootstrap'ом, то есть файлы,
созданные по ходу сессии, в неё не попадут до следующего перечисления. Прежний механизм обновлял её
командами модели — но обновлял неверно (частичным листингом), поэтому обмен идёт «неверное обновление
→ отсутствие обновления». Если устаревание окажется заметным, правильный ответ — инвалидация
структуры сборщиком, а не второй писатель.

**Связано:** P2-64 (тот же ресурс, одновременный конфликт — закрыт), ADR-009 раздел 6
(`project/list_files`), P2-34 (эффективность отбора кандидатов — потребитель структуры).

---

### 66. Структура проекта хранилась в документе сессии, а писалась в копию, которая туда не едет — ✅ ЗАКРЫТО (обнаружено и закрыто 2026-08-11)

**Файлы:** `agent/context/gatherer.py` (`_cached_project_files`, `_bootstrap_project_files`),
`pipeline/stages/agent_loop/tool_processor.py:73` (`_carry_executor_changes` — перенос, которого у
пути сборки контекста нет).

**Обнаружено удалением второго писателя (P2-65).** После него `project_structure` в документе стал
пустым — хотя bootstrap отработал и записал 41 путь (`sess_9f6a653fcf42`). Причина: сборщик пишет в
**рабочую копию** сессии, а на диск она попадает только через явный перенос, который существует на
двух путях — slash-команды и вызовы инструментов модели. У пути сборки контекста переноса нет, и в
докстринге переноса это сказано прямо: «новая мутация в исполнителе, не перечисленная здесь, до диска
не доедет».

**Отсюда вывод, который важнее самого дефекта: записи сборщика не персистировались никогда.** Всё,
что лежало на диске в прежних прогонах (5, 8, 31, 48 путей), писал декоратор — он работал внутри
вызова модели, то есть на пути с переносом. То есть поле в документе всегда содержало **худшее** из
двух значений.

**Решение: признать структуру кэшем, а не состоянием.** Она производна от файловой системы,
восстановима одним перечислением за ~150 мс и не является ни фактом диалога, ни решением
пользователя. По таксономии ADR-008 (журнал / проекции / горячее) ей место не в персистируемом
документе. Сняты и запись, и чтение: структура живёт в процессном кэше `DependencyGraph`, а при его
отсутствии перечисляется заново.

Это третий переезд того же рода и по той же причине: реестр alias'ов терминалов (ADR-007, шаг A),
счётчик alias'ов (P2-58) и фаза turn'а (ADR-008, шаг 2). Каждый раз производное, записанное как
истина, начинало лгать — здесь оно солгало дважды: расхождением 48 против 41 и деградацией 41 → 5.

**Отклонённые варианты.** *Дать сборщику перенос рабочей копии* — третья копия временного шва, от
которого ADR-006 обещал избавиться, и он порождает вопрос инвалидации, которого сейчас нет вовсе.
*Дать сборщику собственную команду* — принципиально верно для состояния, но здесь платить пришлось бы
за право хранить то, что хранить не следует; упёрлись бы в ту же инвалидацию, только позже и дороже.
*Оставить как есть* — запись без эффекта, ровно тот класс, что `terminal_counter` и `set_turn_phase`:
механизм выглядит работающим и не работает.

**Цена, названная прямо:** тёплый старт между процессами потерян — каждый процесс перечисляет файлы
заново (~150 мс, один раз). Взамен исчез класс «после рестарта прочитали деградировавший остаток».

**Совместимость.** Поле `project_structure` в старых документах остаётся и просто игнорируется:
читателя больше нет, миграция не нужна, формат не менялся.

**Гейты:** `test_nobody_writes_structure_into_the_session` (в исходниках нет писателя; проверен
возвратом дефекта), `test_structure_reaches_the_process_cache`, `test_bootstrap_does_not_touch_session_state`.
Тесты, кодировавшие прежнее правило («структура обязана попасть в сессию», seeding структуры через
`config_values`), заменены осознанно: структура приходит перечислением через реестр.

**Связано:** P2-65 (второй писатель — удалён им же), ADR-009 раздел 6 (`project/list_files`),
ADR-007 шаг A и P2-58 (тот же переезд производного из документа), ADR-006 фаза D шаг 4 (перенос
рабочей копии как временный шов).

---

### 67. Вызов в полёте, чей исполнитель умер, не получает `role: tool` никогда — ✅ ЗАКРЫТО (обнаружено и закрыто 2026-08-13, разбор логов)

**Симптом.** В `sess_5fee83e9bc32` — 41 вызов, заявленный в assistant-сообщениях, против 40 ответов
`role: tool`. Без ответа остался `call_034` (`terminal/wait_for_exit`), статус в документе —
`cancelled`.

**Диагноз — владение ответом переживает владельца.** Порядок покадрово:

1. 04:52:16.515 — `call_034` → `in_progress`;
2. 04:52:16.518 — лог `pid 10600` обрывается на `tool_invocation_probe`: процесс умер с вызовом в
   полёте;
3. 04:57:58.558 — `session/cancel` в новом процессе (`pid 12922`) метит вызов `cancelled` и
   **сознательно молчит**: предикат `is_in_flight` пропускает ответ вперёд исполнителю, чтобы тот
   написал правдивый текст (P2-63). Исполнитель — в мёртвом процессе.

**Класс.** Оборотная сторона правки P2-63 и тот же корень, что у `terminal_counter` (P2-58) и эпохи
alias'ов: **процессное состояние, персистированное в документе**. `in_progress` на диске утверждает
про владение то, что после рестарта неверно.

**Почему P1 по последствиям, хотя прогон не упал.** Контракт LLM-API требует `role: tool` на каждый
`tool_call_id` из assistant-сообщения; строгие провайдеры такой запрос отклоняют. Дефект латентный,
переживает рестарт — он лежит на диске. Он же блокировал ADR-008 шаг 4 симметрично P1-60 и P2-63:
журнал говорит `cancelled`, ответа в истории нет, и проекция `history` невыводима.

**Сделано (2026-08-13).** `_cleanup_session_state` (путь загрузки и переключения сессии) метит
**все** нефинальные вызовы, а не только `pending`, и отвечает через единственную дверь
`ToolCallHandler.answer_unexecuted_tool_calls` — та пишет событие журнала
`unexecuted_tool_call_answered` (ADR-008, шаг 4a). Причина в тексте различает факты: «сессия была
переключена» для `pending`, «исполнение прервано, результат потерян» для вызова в полёте.

Закрыто три следствия одной причины: вызов без ответа; вечный `in_progress` в документе сессии, у
которой нет процесса; ответы этой метёлки, не описанные журналом (статус она пишет без контента, а
текст жил только в истории).

Путь `session/cancel` и правка P2-63 не тронуты: владение ответом у **живого** исполнителя остаётся
за ним. Структурное лечение — сделать «в полёте» процессным реестром, как `turn_cancellation` и
`turn_terminals`, — записано в хвост шага 5 ADR-008: оно трогает горячий путь каждого исполнения, а
промах регистрации вернул бы выбор текста P2-63.

**Связано:** P2-63 (предикат, чью оборотную сторону это закрывает), P2-38 (тот же инвариант через
другую дверь), P2-58 (утечка при смерти процесса — тот же класс), ADR-008 шаги 4a/4b.
