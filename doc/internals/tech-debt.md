...
на каждом прогоне: litellm печатает в stdout прямо в файл лога (~41 строка за сессию). У экспортёров
...
**Направления:** найти двойной спавн на стороне клиента (запуск stdio-сервера в
`client/infrastructure`) — совпадение метки до микросекунды говорит о двух `Popen` из одного
места, а не о рестарте.
...
**Связано:** P2-41 (`build_identity` — им и различили процессы), P2-43 (точки входа), ADR-007
(инвариант владения; второй процесс — писатель вне его области), P2-53 (тот же класс: спавн без
владельца жизненного цикла, но с утечкой).
...
### 53. Подпроцессы Web UI не умирают вместе с сервером: 437 осиротевших процессов, 5.5 ГБ — ⬜ ОТКРЫТО (обнаружено 2026-07-31, инспекция процессов)
...
**Динамика за сутки и повышение приоритета до P1 (2026-08-04).** Замеры 2026-08-03 подряд:
110 → 167 → 260 → 349 процессов, 4.8 ГБ RSS. Утечка не разовая и не затухающая — она линейна по
числу запусков клиента, а запусков за день разработки десятки. Сверх памяти есть прямое следствие
для работы: осиротевшие предшественники занимают `port + 1000`, поэтому следующий запуск попадает
в ту же ветку неудачного bind'а — уже ломало воспроизводимость прогонов, на которых держится
приёмка шагов ADR-006/007. **Приоритет: P1** (повышен 2026-08-04; пункт остаётся в разделе P2
физически, чтобы не разрывать нумерацию). Основание — не тяжесть одного случая, а темп и то, что
дефект бьёт по инструменту проверки всех остальных.
...
«разрешения : чтения» на живом прогоне перестаёт быть 0:N.
...
### 58. Мёртвые алиасы терминалов после рестарта: гарантированно провальные вызовы и реестр, который только растёт — ⬜ ОТКРЫТО (обнаружено 2026-08-04, анализ логов)
...
**Направления (решить при взятии):**
1. При чистке реестра на загрузке — привести историю в согласованное состояние: пометить вызовы
   мёртвых терминалов как недействительные, чтобы модель не пыталась их использовать.
2. Отвечать на вызов неизвестного алиаса не ошибкой транспорта, а внятным результатом «терминал не
   существует (сессия перезапущена)» — по образцу P2-36, где отказ без причины давал каскад.
3. Дать терминалам владельца: создавший освобождает. Для Context Manager — `try/finally` вокруг
   bootstrap; в идеале bootstrap вообще без shell (см. P1-56).

**Критерий приёмки:** после рестарта сессии нет провальных вызовов по терминалам прошлого
процесса; за прогон число `terminal/release` сходится с числом `terminal/create`.
