...
> Анализ логов живых прогонов (2026-07-29, ветка `ref/ADR-003-acp-independent-agent-core`): добавлены P2-36 (отказ policy не сообщает модели причину → каскад фантомных терминалов, **закрыт в тот же день**) и P2-37 (ошибки модели и штатные отмены на уровне `error`).
