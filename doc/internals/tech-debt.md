...
**Открытое наблюдение, объяснения не имеет.** У пары `59506`/`59507` метка `build_identity`
совпадает **до микросекунды** (`08:42:59.093995Z` в обоих файлах, при разных `pid` в самих строках).
У утренней пары расхождение было 2 мкс, поэтому в постоянный артефакт это не складывается.
