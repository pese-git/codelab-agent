...
### 62. `session/load` решает судьбу turn'а по «последнему» ожиданию — полем, которым домен решать запрещает — 🟡 ЧАСТИЧНО (обнаружено 2026-08-07, разбор логов; правка и гейт 2026-08-10, статические гейты — живьём не подтверждено)
...
⏳ **Живьём не подтверждено:** сценарий требует загрузки **тем же процессом** при незакрытом
ожидании; прогон с рестартом дефекта не даёт (реестр пуст, ветка отрабатывала «верно» по случайности
— очистка всё равно шла мимо надгробий).
