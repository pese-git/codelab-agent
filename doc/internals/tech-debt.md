...
### 57. Context Manager не окупается: 161–239 чтений на 7–20 файлов, метрика не отражает реальные чтения — 🟨 ЧАСТИЧНО (обнаружено 2026-08-04; 2026-08-17 разложение сделано, поиск сужен до возможности; открытым остаётся сама окупаемость)
...
**Что осталось открытым — собственно окупаемость.** Вопрос «модель перечитала все шесть собранных
файлов сама» этим изменением не затронут: он не про число чтений. Нового механизма он не требует —
`tool_invocation_probe` уже пишет `subject=context` против turn-пути, значит пересечение путей по
subject'ам за turn считается замером на существующих полях.
