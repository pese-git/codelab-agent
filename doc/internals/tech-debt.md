...
### 45. Слияние при конфликте ревизий дублирует историю: два ответа `role: tool` на один вызов — ✅ ЗАКРЫТО (2026-07-31, корень уезжает в фазу D)

**Файлы:** `protocol/session_merge.py:57` (`_common_prefix_length`),
...
в одном прогоне 3766 мс), roadmap Context Manager (слой A).
...
ADR-006 (`TurnState.phase` как `str` — временное решение, `TurnPhase` на стадии b4/b8), P2-42
(ветка orphan на загрузке — потребитель, которому фаза не помогает).

---

### 55. Вызов после fs client-RPC остаётся на диске `pending`, а клиенту ушёл `completed` — ⬜ ОТКРЫТО (обнаружено 2026-07-31, живой прогон транзакции 7)
...
Уклон: вариант 1 плюс пункт 3. Вариант 2 фабрикует состояние, которого не было, ради формального
соответствия матрице.
