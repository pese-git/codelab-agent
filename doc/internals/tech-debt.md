...
### 39. `session/cancel` не останавливает turn: носитель флага уничтожается раньше проверки — ⬜ ОТКРЫТО (обнаружено 2026-07-30, анализ логов)
...
**Направление правки — требует решения (не начинать без него):**

1. *Сигнал на самой сессии* (например, монотонная эпоха turn'а, которую цикл сверяет с той,
...
2. *Не очищать `active_turn` до подтверждения остановки циклом.* Меньше трогает состояние,
   но требует протокола подтверждения между отменой и циклом и оставляет `active_turn`
   живым дольше — риск, что на него сядут другие проверки.
...
### 37. Ошибки модели и штатные отмены логируются как `error` — ✅ ЗАКРЫТО (2026-07-29)
