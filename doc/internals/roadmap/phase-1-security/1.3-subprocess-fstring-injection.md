...
> **Расхождение с текущим кодом (2026-08-04, P2-53).** Снимок выше — исторический. Инвариант
> «подпроцесс не наследует stdio родителя» сохранён, но `DEVNULL` заменён на собственный файл лога
> ребёнка (`~/.codelab/logs/web_ui-<pid родителя>.log`): без него у утечки подпроцессов не было
> канала наблюдаемости. Сверх этого ребёнок получает `CODELAB_PARENT_PID` и завершается сам, когда
> родитель исчезает. Актуальное состояние — в `shared/web_ui.py`.
