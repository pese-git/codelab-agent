...
> **Статус актуализирован:** 2026-07-10 (ветка `develop`). Легенда: ✅ Done · 🟡 Частично · ⬜ Открыто.
> Статусы проставлены по фактической сверке с кодом; для непроверенных пунктов оставлено ⬜.

---

## Фаза 0 — Немедленно (до следующего деплоя)

| Документ | Проблема | Оценка | Статус |
|---|---|---|---|
...
| [1.2 Path Traversal](phase-1-security/1.2-path-traversal.md) | `startswith` вместо `is_relative_to` | 1 ч | 🟡 добавлен слой валидации путей + тесты; остаток: `filesystem.py:50` всё ещё `startswith` вместо `is_relative_to` |
...
| _1.6 — пропуск нумерации (документ отсутствует)_ | — | — | — |
...
| [2.8 Миграция DI на dishka](phase-2-architecture/2.8-di-dishka-migration.md) | Самописный контейнер не делает DI | 2 дня | ✅ `dishka` в `server/di.py`, `client/infrastructure/container_factory.py`, `providers.py` |
...
| [3.4 structlog без f-strings](phase-3-quality/3.4-structlog-fstrings.md) | f-strings разрушают структурированность логов | 2 ч | ✅ миграция на structlog kwargs проведена |
...
| [3.6 Тесты для security-путей](phase-3-quality/3.6-security-tests.md) | Нет тестов path traversal и shell injection | 1 день | ✅ `tests/server/tools/test_filesystem_path_validation.py` |
...
| [3.9 Переименование TestViewModel](phase-3-quality/3.9-test-viewmodel-rename.md) | PytestCollectionWarning | 30 мин | ✅ переименовано (`tests/client/test_dishka_di_container.py`) |
...
> **Прогресс (2026-07-10):** закрыто ✅ 2.8, 3.4, 3.6, 3.9; частично 🟡 1.2. Остальные пункты не
> перепроверялись — статус ⬜ означает «не подтверждено», а не обязательно «не сделано».

Полный аудит технического долга: [`../tech-debt.md`](../tech-debt.md)
