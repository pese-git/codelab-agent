...
class TurnLifecycleManager:
    async def open_turn(self, session: DomainSession) -> list[Notification]:
        """Открыть turn, отправить session/started."""
        ...
    
    async def close_turn(self, session: DomainSession, stop_reason: str) -> list[Notification]:
...
class EventHistoryWriter:
    """Единственный писатель журнала (turn-путь)."""

    def save_user_message(self, session: DomainSession, content: dict) -> None: ...
    def save_agent_message(self, session: DomainSession, content: dict) -> None: ...
    def save_tool_call(self, session: DomainSession, tool_call_id: str, ...) -> None: ...
    def save_tool_call_update(self, session: DomainSession, tool_call_id: str, ...) -> None: ...
    def save_plan(self, session: DomainSession, entries: list[dict]) -> None: ...
    def save_session_info_update(self, session: DomainSession, *, title, updated_at) -> None: ...


class SessionReplayer:
    """Воспроизведение накопленной истории (путь session/load)."""

    def replay_history(self, session: DomainSession) -> list[ACPMessage]: ...
    def replay_latest_plan(self, session: DomainSession) -> ACPMessage | None: ...
```
