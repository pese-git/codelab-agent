...
- **`ContextGatherer` / `ACPContextGatherer`** — по профилю задачи подбирает файлы:
  ...
  границы протокола.
- **`DependencyGraph`** — строит граф импортов и добавляет к найденным файлам их
  зависимости/зависимых. Нужен, чтобы в контекст попал не только изменяемый файл, но и
  то, что с ним связано (интерфейсы, вызывающий код).
...
- **`ChildSessionManager` / `DefaultChildSessionManager`** — создаёт изолированные
  дочерние сессии (свой `agent_scope` и эпоха) и собирает их результат. Изоляция —
  выбор по умолчанию: чистые границы и предсказуемый бюджет.
- **`process_subagent_response()`** — возвращает родителю **суммаризованный** результат
  субагента (`SubagentResult`, `ContextType.AGENT_REPORT`), а не сырой контекст. При
  сбое суммаризации — усечённый сырой результат (мягкая деградация).
