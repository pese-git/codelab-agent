...
graph TB
    ...
    subgraph CLIENT["CodeLab Client (Clean Architecture + MVVM)"]
        ...
        subgraph DOMAIN["Domain Layer"]
            ...
            EVENTS["19 Domain Events<br/>Session, Prompt, Permission<br/>Error, ToolCall, Config"]
        end
...
- Receive Loop: `ReceiveLoopDegradedEvent`, `ReceiveLoopFailedEvent`
- Config: `ConfigOptionUpdatedEvent`

**Детальное описание Domain Events:**

| Событие | Описание | Поля |
|---------|----------|------|
| `SessionCreatedEvent` | Новая сессия создана | `session_id`, `server_host`, `server_port` |
| `SessionInitializedEvent` | Сессия инициализирована | `session_id`, `capabilities` |
| `SessionClosedEvent` | Сессия закрыта | `session_id`, `reason` |
| `SessionLoadedEvent` | Сессия загружена | `session_id`, `from_storage` |
| `PromptStartedEvent` | Prompt turn начался | `session_id`, `prompt_text` |
| `PromptCompletedEvent` | Prompt turn завершился | `session_id`, `stop_reason` |
| `PromptCancelledEvent` | Prompt turn отменён | `session_id`, `reason` |
| `PermissionRequestedEvent` | Запрошено разрешение | `session_id`, `action`, `resource`, `permission_id` |
| `PermissionGrantedEvent` | Разрешение дано | `session_id`, `permission_id`, `action` |
| `PermissionDeniedEvent` | Разрешение отклонено | `session_id`, `permission_id`, `action` |
| `ErrorOccurredEvent` | Произошла ошибка | `error_message`, `error_type`, `session_id` |
| `ConnectionLostEvent` | Соединение потеряно | `session_id`, `error_message` |
| `ConnectionRestoredEvent` | Соединение восстановлено | `session_id` |
| `ToolCallStartedEvent` | Инструмент начал выполняться | `session_id`, `tool_name`, `tool_use_id` |
| `ToolCallCompletedEvent` | Инструмент завершил выполнение | `session_id`, `tool_name`, `tool_use_id`, `result` |
| `ToolCallFailedEvent` | Выполнение инструмента не удалось | `session_id`, `tool_name`, `tool_use_id`, `error_message` |
| `ReceiveLoopDegradedEvent` | Receive loop работает с деградацией | `session_id`, `restarts_count`, `consecutive_errors`, `last_error` |
| `ReceiveLoopFailedEvent` | Receive loop полностью остановился | `session_id`, `restarts_count`, `final_error`, `suggestion` |
| `ConfigOptionUpdatedEvent` | Конфигурационная опция обновлена | `session_id`, `config_options` |
...
#### Presentation Chat подсистема (`client/presentation/chat/`)

Подсистема для управления состоянием чата и обработки session/update событий. Реализует Strategy pattern для handlers и Port/Adapter pattern для persistence.

```mermaid
graph TB
    subgraph "Presentation Chat Subsystem"
        CVM[ChatViewModel]
        CSS[ChatSessionState]
        DISP[SessionUpdateDispatcher]
        
        subgraph "Handlers (4)"
            MCH[MessageChunkHandler]
            TCH[ToolCallHandler]
            PUH[PlanUpdateHandler]
            COH[ConfigOptionHandler]
        end
        
        subgraph "Executors (3)"
            FSCE[FsCallbackExecutor]
            TBCE[TerminalCallbackExecutor]
            TEA[TerminalExecutorAdapter]
        end
        
        subgraph "Persistence"
            FCP[FileChatPersistence]
        end
        
        subgraph "Contracts"
            SUH[SessionUpdateHandler Protocol]
            CPP[ChatPersistencePort Protocol]
            CUS[ChatUpdateSink Protocol]
        end
    end
    
    CVM --> CSS --> DISP
    DISP --> MCH & TCH & PUH & COH
    CVM --> FSCE & TBCE & TEA
    CVM --> FCP
    MCH & TCH & PUH & COH -.-> SUH
    FCP -.-> CPP
...
|-----------|------|----------|
| `ChatSessionState` | `chat_session_state.py` | Состояние чата для каждой сессии: messages, tool_calls, streaming_text, replay_updates |
| `ChatUpdateContext` | `context.py` | Контекст для обработки session/update: session_id, state, sink, plan_vm, event_bus |
| `SessionUpdateDispatcher` | `dispatcher/session_update_dispatcher.py` | Маршрутизация session/update к handlers по update_type (O(1) поиск) |

**Handlers (4):**

| Handler | Update Types | Описание |
|---------|--------------|----------|
| `MessageChunkHandler` | `agent_message_chunk`, `user_message_chunk` | Добавляет текст в streaming_text или messages |
| `ToolCallHandler` | `tool_call`, `tool_call_update`, `tool_call_result` | Управление списком tool_calls: создание, обновление статуса, логирование результатов |
| `PlanUpdateHandler` | `plan` | Форматирование плана и обновление PlanViewModel |
| `ConfigOptionHandler` | `config_option_update` | Публикация ConfigOptionUpdatedEvent в EventBus |

**Executors (3):**

| Executor | Описание |
|----------|----------|
| `FsCallbackExecutor` | Async-safe операции с файлами через `asyncio.to_thread()`. Sandbox для предотвращения path traversal. Методы: `read_file()`, `write_file()` |
| `TerminalCallbackExecutor` | Управление жизненным циклом терминалов. Кэширование состояния. Методы: `create_terminal()`, `get_output()`, `wait_for_exit()`, `release_terminal()`, `kill_terminal()` |
| `TerminalExecutorAdapter` | Адаптер для TerminalExecutor из infrastructure. Реализует TerminalExecutorPort |

**Persistence:**

| Компонент | Описание |
|-----------|----------|
| `FileChatPersistence` | Файловое хранилище истории чата в JSON. Async-safe через `asyncio.to_thread()`. Методы: `save_messages()`, `load_messages()`, `load_replay_updates()` |

**Contracts (Protocol интерфейсы):**
...
class ChatPersistencePort(Protocol):
    async def save_messages(self, session_id: str, messages: list[dict[str, str]], replay_updates: list[dict[str, Any]] | None = None) -> None: ...
...
class ChatUpdateSink(Protocol):
    ...
    def sync_streaming(self, session_id: str, streaming_text: str, is_streaming: bool) -> None: ...
```
...
from codelab.client.presentation.chat.dispatcher.session_update_dispatcher import SessionUpdateDispatcher
from codelab.client.presentation.chat.handlers.message_chunk_handler import MessageChunkHandler
from codelab.client.presentation.chat.handlers.tool_call_handler import ToolCallHandler
from codelab.client.presentation.chat.handlers.plan_update_handler import PlanUpdateHandler
from codelab.client.presentation.chat.handlers.config_option_handler import ConfigOptionHandler
from codelab.client.presentation.chat.executors.fs_callback_executor import FsCallbackExecutor
from codelab.client.presentation.chat.executors.terminal_callback_executor import TerminalCallbackExecutor
from codelab.client.presentation.chat.persistence.file_chat_persistence import FileChatPersistence
...
# Создаём executors
fs_executor = FsCallbackExecutor(base_path=Path("/path/to/workspace"))
terminal_executor = TerminalCallbackExecutor(executor=terminal_executor_port)

# Создаём persistence
persistence = FileChatPersistence(history_dir=Path("/path/to/history"))

# Создаём ChatViewModel
chat_vm = ChatViewModel(
    ...
    fs_callback_executor=fs_executor,
    terminal_callback_executor=terminal_executor,
)
```
