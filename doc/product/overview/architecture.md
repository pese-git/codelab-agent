...
## Журнал сессии и проекции

Единственный источник фактов диалога на сервере — **журнал** (`domain/journal.py`):
упорядоченная последовательность доменных событий. Всё остальное из него **выводится**
(ADR-008).

| | Журнал (`SessionJournal`) | Проекции |
|---|---|---|
| **Что это** | факты диалога в терминах домена | виды: `history` для LLM, `tool_calls` — реестр вызовов |
| **Кто пишет** | `EventHistoryWriter` (turn-путь) — единственный писатель | никто: проекции считаются |
| **Форма** | домен, ничего не знает о `sessionUpdate` и camelCase | `history_projection.py`, `tool_call_projection.py` |
| **Гранулярность** | сообщение, а не чанк потока (замер на живом документе) | — |
| **Носитель** | JSONL, дописывается | пересчитываются, на диск не едут как истина |

Ключевой инвариант: **проекцию можно выбросить и пересчитать**, поэтому в модулях
проекций нет ни состояния, ни обращений к носителю. Форму wire (`events_history`,
`session/update`) знает `mapping/journal_mapper.py` — единственный владелец этой границы.

`SessionReplayer` воспроизводит накопленную историю на `session/load`; `session_info_update`
пишется в журнал, но не реплеится — по ACP это патч-канал метаданных, а не conversation.
