...
1. **Загрузка структуры проекта** — из процессного кэша, а при его отсутствии — перечислением
   возможностью `project/list_files` (команду формирует сервер). В документе сессии структура не
   хранится: она производна от файловой системы и восстановима за ~150 мс;
2. **Отбор кандидатов** — `target_modules` + `search_terms` → `search()`. Совпадения по пути
   ищутся в структуре, по содержимому — возможностью `project/search_content` (команду так же
   формирует сервер, вызывающий передаёт только термин). Файлы ради поиска не читаются: поиск
   возвращает пути;
