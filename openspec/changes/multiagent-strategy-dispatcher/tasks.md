...
- [ ] 1.4 Реализовать приоритет: `context.meta["routing_mode"]` → `config_values["_routing_mode"]` → `"single"`
...
### 3. Config Option `_routing_mode`

> **Существующий код:** `SessionState.config_values: dict[str, str]` уже поддерживает произвольные ключи. Не нужно менять модель — `_routing_mode` хранится в `config_values`.

- [ ] 3.1 ~~Обновить `state.py`~~ — ПРОПУСТИТЬ: `config_values` уже поддерживает произвольные ключи
- [ ] 3.2 Обновить `handlers/config.py` — обработка `set_config_option` для `_routing_mode` с валидацией значений (single, multi_orchestrated, multi_choreographed, hierarchical)
- [ ] 3.3 Написать тесты: set_config_option _routing_mode → сохраняется в config_values
...
- [ ] 5.1 Обновить обработчик slash command для поддержки `/mode` routing (опционально)
- [ ] 5.2 Установить `context.meta["routing_mode"]` при slash command
