...
Система MUST кэшировать содержимое файлов для каждой сессии, чтобы устранить дублирующиеся вызовы ACP `read_file()`.

#### Scenario: Попадание в кэш при повторном чтении
- **WHEN** файл читается через `fs/read`, а затем читается снова в той же сессии
- **THEN** второе чтение возвращает содержимое из `FileContentCache` без ACP RPC, логирует debug `file_cache_miss` при промахе кэша, без лога при попадании (hit)
...
- **THEN** система публикует сигнал изменения в единый источник истины (точка интеграции Фаза 2 ↔ Фаза 4)

### Requirement: SessionFileCacheRegistry управляет жизненным циклом кэша
Система MUST управлять файловыми кэшами для каждой сессии с правильным жизненным циклом.
...
Система MUST оборачивать `ToolExecutor` через `FileCacheDecorator` для перехвата `fs/read` и `fs/write`.
...
Система MUST сжимать код до сигнатур используя AST анализ, сохраняя структуру.
...
Система MUST корректно обрабатывать файлы на неподдерживаемых языках.
...
Система MUST точно подсчитывать токены используя tiktoken с fallback.
...
Система MUST представлять каждый элемент контекста как `ContextItem` с приоритетом.
...
Система MUST обеспечивать детерминированный вывод для `CodeSkeletonizer` и `FileContentCache` для поддержания стабильности кэша.
