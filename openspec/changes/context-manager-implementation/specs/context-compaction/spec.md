...
Система MUST выполнять компактирование в три фазы: Prune → Skeletonize → Summarize.
...
Система MUST удалять старые сообщения tool outputs используя FIFO, сохраняя первые 2 и последние N сообщений.
...
Система MUST сжимать файлы кода используя `CodeSkeletonizer` во время компактирования.
...
Система MUST суммаризировать разговор используя LLM, когда Prune + Skeletonize недостаточны.
...
Система MUST NOT вытеснять элементы с `priority >= 10` во время компактирования.
...
Система MUST выполнять жёсткое усечение, если payload всё ещё превышает бюджет после трёх фаз.
...
Система MUST санитизировать осиротевшие tool сообщения перед формированием `PayloadEnvelope`.
...
Система MUST эмитировать метрики для коэффициента компактирования, длительности и деградации.
...
Система MUST гарантировать, что payload помещается в `max_context_tokens - reserved_tokens`.
