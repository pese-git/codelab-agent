...
- [ ] 1.3 Реализовать метод `Agent.call()`: messages, tools, config, parent_span → AgentResult
...
- [ ] 1.6 Реализовать сборку `AgentResult`: text, tool_calls, usage (из LLM response), stop_reason, agent_name
- [ ] 1.7 Реализовать отмену: отслеживание asyncio.Task, возврат cancelled AgentResult

### 2. Регистрация в EventBus

- [ ] 2.1 Реализовать метод `_handle_request()`: конвертация AgentRequest → AgentResponse
...
### 5. Маппинг имён инструментов

- [ ] 5.1 Реализовать маппинг имён инструментов: ACP `/` → LLM `_` (сохранение из NaiveAgent)
- [ ] 5.2 Реализовать извлечение плана (сохранение из NaiveAgent)
- [ ] 5.3 Написать тесты: корректность маппинга имён инструментов
