...
## Purpose
TBD - created by archiving change acp-independent-agent-core. Update Purpose after archive.
## Requirements
...
#### Scenario: Семантика ошибок доставки (Фаза 3, контракт)
- **WHEN** адаптер `UpdateSink` не может доставить событие (transport error / no callback)
- **THEN** адаптер MUST логировать ошибку и буферизовать событие для последующего replay
- **THEN** ядро не должно знать про факт ошибки доставки (fire-and-forget на уровне ядра)
- Реализация этого контракта — отдельный change после Фазы 4 (Фаза 3 фиксирует только **требования**, не реализацию унификации буферизации)
