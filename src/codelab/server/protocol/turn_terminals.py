...
метода «забыть сессию»), а дисконнект вообще не имеет адресата для RPC. Освобождение
принадлежит трём путям, где turn кончается потому, что модель закончила: штатное
завершение, отмена и ошибка пайплайна.
