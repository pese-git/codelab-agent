...
ACP_CLIENT_INFO = MCPClientInfo(name="codelab", version="1.0.0")
...
class MCPClient:
    ...
    async def set_roots(self, roots: list[MCPRoot]) -> None:
        ...
        if self._state == MCPClientState.READY and self._transport:
            ...
            if old_uris != new_uris:
                ...
                await self._transport.send_notification(method="notifications/roots/list_changed")
    ...
    def _handle_roots_list(self) -> list[dict[str, Any]]:
        ...
        """
        return [{"uri": root.uri, "name": root.name} for root in self._roots]

    async def _handle_roots_list_request(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        logger.debug("Handling roots/list request from MCP server: %s", self.config.name)
        ...
        try:
            ...
            if hasattr(self._transport, "register_request_handler"):
                self._transport.register_request_handler(
                    "roots/list", self._handle_roots_list_request
                )
                logger.debug(
                    "Registered roots/list request handler for server: %s", self.config.name
                )
    ...
    async def initialize(self) -> MCPCapabilities:
        ...
        try:
            ...
            await self._transport.send_notification(method="notifications/initialized")
            ...
            self._state = MCPClientState.FAILED
            raise MCPInitializeError(f"Initialize failed for {self.config.name}: {e}") from e
        except Exception as e:
            self._state = MCPClientState.FAILED
            raise MCPInitializeError(f"Initialize error for {self.config.name}: {e}") from e
    ...
    async def list_tools(self) -> list[MCPTool]:
        ...
        if self._capabilities and self._capabilities.tools is None:
            logger.debug("MCP server %s does not support tools", self.config.name)
            return []
        ...
        try:
            ...
            logger.info("MCP server %s provides %d tools", self.config.name, len(self._tools))
    ...
    async def list_resources(self) -> list[MCPResource]:
        ...
        if self._capabilities and self._capabilities.resources is None:
            logger.debug("MCP server %s does not support resources", self.config.name)
            return []
        ...
        try:
            ...
            logger.info(
                "MCP server %s provides %d resources", self.config.name, len(self._resources)
            )
    ...
    async def list_resource_templates(self) -> list[MCPResourceTemplate]:
        ...
        """
        if self._state != MCPClientState.READY:
            raise MCPClientError(f"Cannot list resource templates in state {self._state}")
        ...
        if self._capabilities and self._capabilities.resources is None:
            logger.debug("MCP server %s does not support resources/templates", self.config.name)
            return []

        logger.debug("Requesting resource templates list from: %s", self.config.name)
    ...
    async def read_resource(self, uri: str) -> MCPReadResourceResult:
        ...
        logger.debug("Reading MCP resource: %s (server=%s)", uri, self.config.name)
        ...
        try:
            ...
            logger.debug(
                "MCP resource %s read successfully (%d contents)", uri, len(result.contents)
            )
    ...
    async def list_prompts(self) -> list[MCPPrompt]:
        ...
        if self._capabilities and self._capabilities.prompts is None:
            logger.debug("MCP server %s does not support prompts", self.config.name)
            return []
        ...
        try:
            ...
            logger.info("MCP server %s provides %d prompts", self.config.name, len(self._prompts))
    ...
    async def get_prompt(
        ...
        logger.debug("Getting MCP prompt: %s (server=%s)", name, self.config.name)
        ...
        try:
            ...
            logger.debug(
                "MCP prompt %s retrieved successfully (%d messages)", name, len(result.messages)
            )
    ...
    async def call_tool(
        ...
        logger.debug("Calling MCP tool: %s.%s with args=%s", self.config.name, name, arguments)
        ...
        try:
            ...
            if result.is_error:
                ...
                logger.debug("MCP tool %s.%s completed successfully", self.config.name, name)

            return result

        except (StdioTransportError, HttpTransportError, SseTransportError) as e:
            raise MCPToolCallError(f"Tool call {name} failed: {e}") from e
    ...
    def register_progress_callback(self, callback: Callable) -> None:
        ...
        logger.debug("Registered progress callback for: %s", self.config.name)
    ...
    async def start_notification_processing(self) -> None:
        ...
        self._notification_task = asyncio.create_task(
            self._process_notifications(), name=f"mcp_notification_processor_{self.config.name}"
        )

        logger.debug("Started notification processing for: %s", self.config.name)
    ...
    async def _process_notifications(self) -> None:
        ...
        while self._state == MCPClientState.READY:
            try:
                ...
                logger.debug(
                    "Processing MCP notification: method=%s server=%s", method, self.config.name
                )
                ...
                if method == "notifications/progress":
                    await self._handle_progress_notification(notification.get("params", {}))
    ...
    async def handle_notification(self, notification_data: dict) -> None:
        ...
        logger.debug("Received MCP notification: method=%s server=%s", method, self.config.name)
    ...
    async def _handle_progress_notification(self, params: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning("Failed to parse progress notification from %s: %s", self.config.name, e)
            return
        ...
        for callback in self._progress_callbacks:
            try:
                ...
                logger.error("Error in progress callback for %s: %s", self.config.name, e)
    ...
    async def stop_notification_processing(self) -> None:
        ...
        if self._notification_task is not None:
            ...
            logger.debug("Stopped notification processing for: %s", self.config.name)
