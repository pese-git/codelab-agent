...
class SessionView(Protocol):
    ...
    (`protocol.state.SessionDocument`) удовлетворяет порт структурно — чтение идёт
...
def writable_session(session: Any) -> Any:
    """Носитель состояния из того, что пришло по порту.

    Порт `SessionView` read-only, и это осознанно: turn не вправе писать в
    долгоживущую копию (ADR-006, шаг 1 фазы D). Но два потребителя порта состояние
    всё-таки меняют — инструменты (реестр терминалов, `set_config_value`) и
    bootstrap Context Manager'а, — и оба получают ровно тот объект, который держит
    вызывающий. До перевода turn-пути на доменный агрегат это работало случайно:
    и wire-DTO, и проекция отвечали на одни и те же имена. После — только носитель,
    и молчаливая деградация проявилась в проде (`'DomainSessionView' object has no
    attribute ...`, 54 подряд).

    Развилка по атрибуту, а не по типу: порт структурный, и знать про конкретную
    проекцию (`mapping.session_view`) ни ядру, ни `tools/` нельзя. Возврат объекта
    как есть — для вызывающих, которые уже держат носитель (turn-путь).

    Это шов, а не решение: сами записи мимо владельца состояния снимаются вместе с
    шагами 4-5 фазы D, когда мутации станут командами в своих транзакциях.
    """
    return getattr(session, "aggregate", None) or session
