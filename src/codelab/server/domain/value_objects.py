...
class ToolInvocationSubject(enum.StrEnum):
    """От чьего имени исполняется инструмент (ADR-009).

    Разрешение — свойство инвокации, а не состояния сессии (ADR-008 раздел 7),
    поэтому у инвокации обязан быть субъект: политика есть функция от него.
    Именно его отсутствие и есть P1-56 — проверку делает вызывающий, и
    вызывающий без проверки её не делает.

    Субъект передаётся **явно**, а не выводится реестром из состояния: ambient
    authority («догадаться, кто звонит») — это тот же дефект в другой форме.

    `UNKNOWN` — не значение по умолчанию для удобства, а **сигнал**: путь, не
    назвавший себя, обязан быть виден в замере, иначе новый вызывающий снова
    окажется мимо гейта молча.
    """
